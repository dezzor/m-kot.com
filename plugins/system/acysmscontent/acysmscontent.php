...
class plgSystemAcysmsContent extends JPlugin
{
	...
	function plgSystemAcysmsContent(&$subject, $config){
	 ...
	 function onACYSMSGetTags(&$tags) {
		...
		$tags['joomlacontent'] = new stdClass();
		$tags['joomlacontent']->name = JText::_('SMS_INSERT_CONTENT');

		$checkbox .= '<div style="margin:15px 0 15px 15px"><input type="checkbox" id="title" name="checkboxInsert" value="title" checked="checked"><label for="title">'.JText::_('SMS_TITLE').'</label> ';
		...
		$wrap = '<br /><div id="wrap" style="display:none">'.JText::sprintf('SMS_ONLY_FIRST_X_CHARACTERS','<input type="text" id="wrapCharacterNumber" class="inputbox" style="width:20px;"/>' ).'</div>';
		...
		$articleSelection = '<div id="articleSelection">'.JText::_('SMS_SELECT_ARTICLE').' : <a class="modal"  onclick="var contentToTrigger = getContentDropwDownValue(); window.acysms_js.openBox(this,\'index.php?option=com_acysms&tmpl=component&ctrl='.$ctrl.'&task=tag&fctplug=chooseArticle&content=\'+contentToTrigger);return false;" rel="{handler: \'iframe\', size: {x: 800, y: 500}}"><img class="icon16" src="'.ACYSMS_IMAGES.'icons/icon-16-edit.png" /></a></div><br />';
			...
			<tbody>
				<?php
					...
					for($i = 0,$a = count($rows);$i<$a;$i++){
						$row = $rows[$i];
				?>
					<tr id="content<?php echo $row->listingId?>" class="<?php echo "row$k"; ?>" onclick="addArticle(<?php echo $row->listingId.",'row$k'"?>);" style="cursor:pointer;">
	...
	function onACYSMSdisplayParamsAutoMessage_content(){
		...
		$dropdownTrigger = JHTML::_('select.genericlist', $triggerData, 'data[message][message_receiver][auto][content][trigger]', 'style="width:80px;" class="inputbox"','value','text', '');
		...
		$dayField = JHTML::_('select.genericlist',   $days, 'data[message][message_receiver][auto][content][generatingdate][day]', 'style="width:50px;" class="inputbox"','value','text', ACYSMS::getDate(time(),'d'));
		...
		$hourField = JHTML::_('select.genericlist',$hours, 'data[message][message_receiver][auto][content][generatingdate][hour]', 'style="width:50px;" class="inputbox"','value','text', ACYSMS::getDate(time(),'H'));
		$minField = JHTML::_('select.genericlist',$min, 'data[message][message_receiver][auto][content][generatingdate][min]', 'style="width:50px;" class="inputbox"','value','text', ACYSMS::getDate(time(),'i'));
	...
	public function onContentAfterSave($context, &$article, $isNew){
		$this->_articleModification($article);
	}
	...
	private function _articleModification($article){
		JPluginHelper::importPlugin('acysms');
		$dispatcher = JDispatcher::getInstance();
		...
		foreach($allMessages as $oneMessage){
			...
			if($generate){
				...
				$oneMessage->message_receiver['auto']['content']['generatingdate']['min'] = ACYSMS::getDate(time(),'i');
