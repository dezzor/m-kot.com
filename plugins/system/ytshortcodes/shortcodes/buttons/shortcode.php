...
function buttonsYTShortcode($atts, $content = null){
	...
	if($icon_position == "left")
	{
		return '<a class="yt-clearfix buttons-style-'.$style.' ytyt_btn'.
			...
			' " style="border-radius:'.$radius.'; '.$background.';'.$dropshadow.'; '.$color.'; '.$center.' ; '.$stylecss.'" href="'.$link.'" target="'.$target.'" data-placement="'.$position.'" title="'.$title.'">'.'<span class="span-buttons"><span>'.$yt_btn_icon. parse_shortcode(str_replace(array("<br/>", "<br>", "<br />", "<p>", "</p>"), " ",$content)).'</span> '.$subtitle .'</span></a>';
	}else{
		return '<a class="yt-clearfix buttons-style-'.$style.' ytyt_btn'.
			(($type != '') ? ' yt_btn-' . $type : '').
			(($size != '') ? ' yt_btn-' . $size : '') .
			(($full == 'yes') ? ' yt_btn-block' : '') .
			' " style="border-radius:'.$radius.'; '.$background.';'.$dropshadow.'; '.$color.'; '.$center.' ; '.$stylecss.'" href="'.$link.'" target="'.$target.'" data-placement="'.$position.'" title="'.$title.'">'.'<span class="span-buttons"><span>'. parse_shortcode(str_replace(array("<br/>", "<br>", "<br />", "<p>", "</p>"), " ",$content)) .$yt_btn_icon. '</span> '.$subtitle .'</span></a>';
	}
