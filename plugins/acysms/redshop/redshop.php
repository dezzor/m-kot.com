...
{
	function plgAcysmsRedshop(&$subject, $config){
	...
	function onACYSMSDisplayFiltersSimpleMessage($componentName, &$filters){
		...
		if($app->isAdmin() || (!$app->isAdmin() && $helperPlugin->allowSendByGroups('redshoporder'))) $filters['redshoporder'] = $newFilter;
	}
	...
	function onACYSMSGetTags(&$tags) {
		...
		if(!$integration->isPresent()) return;
	 	...
	 	$tags['redshopuser'] = new stdClass();
		$tags['redshopuser']->name = JText::sprintf('SMS_X_USER_INFO','RedShop');
		$db = JFactory::getDBO();

		$tableFields = array();
		$tableFields = acysms_getColumns('#__redshop_users_info');

		$tags['redshopuser']->content = '<table class="adminlist table table-striped table-hover" cellpadding="1" width="100%"><tbody>';
		$k = 0;
		foreach($tableFields as $oneField => $fieldType){
			$tags['redshopuser']->content .= '<tr style="cursor:pointer" onclick="insertTag(\'{redshopuser:'.$oneField.'}\')" class="row'.$k.'"><td>'.$oneField.'</td></tr>';
			$k = 1-$k;
		}
		$tags['redshopuser']->content .= '</tbody></table>';


	 	$tags['rshopCoupon'] = new stdClass();
		$tags['rshopCoupon']->name = JText::sprintf('SMS_X_COUPON','RedShop');
		$prefix = 'rshop';
		...
		$tags['rshopCoupon']->content = '<table class="adminlist table table-striped table-hover" cellpadding="1" width="100%"><tbody>';
		$tags['rshopCoupon']->content .= '<tr> <th>'.JText::_('COM_REDSHOP_COUPON_CODE').'</th> <td colspan="2"><input id="'.$prefix.'coupon" type="textbox" maxlength="32" value="[key][user]"/> </td>';
		$tags['rshopCoupon']->content .= '<tr> <th>'.JText::_('COM_REDSHOP_COUPON_VALUE').'</th> <td><input id="'.$prefix.'couponvalue" size="5" type="textbox"/> ';
		$tags['rshopCoupon']->content .= $listCouponValue.'</td></tr>';
		$tags['rshopCoupon']->content .= '<tr> <th>'.JText::_('SMS_EXPIRY_DATE').'</th> <td colspan="2">'.$radioListExpiry.'</td> </tr>';
		$tags['rshopCoupon']->content .= '<tr id="'.$prefix.'expiryDate"> <th>'.JText::_('SMS_FIELD_DATE').'</th> <td colspan="2">'.$timeField[0].$timeField[1].$timeField[2].'</td></tr>';
		$tags['rshopCoupon']->content .= '<tr id="'.$prefix.'expiryDelay" style="display:none"> <th>'.JText::_('SMS_DELAY').'</th> <td colspan="2">'.$displayDelay[0].$displayDelay[1].'</td></tr>';
		$tags['rshopCoupon']->content .= '<tr><td colspan="3"> <input type="button" value="'.JText::_('SMS_INSERT_COUPON').'" onclick="createTagrshopCoupon(\''.$prefix.'\')"/> </td></tr>';
		$tags['rshopCoupon']->content .= '</tbody></table>';
		?>
		...
		function createTagrshopCoupon(prefix)
