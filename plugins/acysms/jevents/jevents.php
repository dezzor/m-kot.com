...
class plgAcysmsJevents extends JPlugin {
	...
	function plgAcysmsJevents(&$subject, $config) {
	...
	?>
		...
		function addEventTag(contentid,rowClass)
		{
			...
			for(var i in selectedContents)
			{
				...
				{
					if(wrapNumber)
						tag = tag + "{" + "<?php echo $tagName; ?>:"+wrapNumber+ "| "+"<?php echo $nextJeventsParameter; ?>:"+selectedContents[i]+otherinfo+"}";
					else
...
<?php }
	...
	function onACYSMSGetTags(&$tags) {
		...
		$tags['jevents'] = new stdClass();
		$tags['jevents']->name = JText::_('SMS_JEVENTS');

		$nbEvent = array ();
		for ($i = 1; $i <= 25; $i += 1) {
			$nbEvent[] = JHTML::_('select.option', $i, $i);
		}
		$nbEventSelection = JHTML::_('select.genericlist', $nbEvent, "name", "", 'value', 'text', 'all', 'nbEvent_jevents');
		...
		$articleSelection = '<div>'.JText::_('SMS_SELECT_EVENT').' <a class="modal" style="cursor:pointer" onclick="window.acysms_js.openBox(this,\'index.php?option=com_acysms&tmpl=component&ctrl='.$ctrl.'&task=tag&fctplug=chooseEvents_jevents\');return false;" rel="{handler: \'iframe\', size: {x: 800, y: 500}}"><img class="icon16" src="'.ACYSMS_IMAGES.'icons/icon-16-edit.png" /></a></div><br />';
		$resultCategory = '<div>'.JText::sprintf('SMS_SELECT_X_EVENTS_FILTERED_X',$nbEventSelection,JText::_('JEV_EVENT_CATEGORY')).' <a id="listCatLoc_jevents" class="modal"  onclick="window.acysms_js.openBox(this,\'index.php?option=com_acysms&tmpl=component&ctrl='.$ctrl.'&task=tag&fctplug=chooseCategoryEvents_jevents&context=tags\');return false;" rel="{handler: \'iframe\', size: {x: 800, y: 500}}"><img class="icon16" src="'.ACYSMS_IMAGES.'icons/icon-16-edit.png" /></a></div><br />';
		...
		$tags['jevents']->content = "<table class='adminlist table table-striped table-hover' cellpadding='1' width='100%'><tbody>";
		$tags['jevents']->content .= '<tr><th colspan="4">'.JText::_('SMS_INSERT_INFORMATION').'</th></tr>';
		$tags['jevents']->content .= "<tr><td>".$typeOfInsert[0].'</td><td>'.$typeOfInsert[1].'</td><td>'.$typeOfInsert[2].'</td><td>'.$typeOfInsert[3]."</td></tr>";
		$tags['jevents']->content .= '<tr><th colspan="4">'.JText::_('SMS_EVENT_SELECTION').'</th></tr>';
		$tags['jevents']->content .= '<tr><td colspan="4">'.$articleSelection.'</td></tr>'; // select one event
		$tags['jevents']->content .= '<tr><td colspan="4">'.$resultCategory.$buttonInsertTag; //select multiple events (the next XX)+button
		$tags['jevents']->content .= '</td></tr></tbody></table>';




		$tags['jeventsauto'] = new stdClass();
		$tags['jeventsauto']->name = JText::sprintf('SMS_EVENTS_AUTOMESSAGE_TAGS_X','JEventsPro');
		...
		$tags['jeventsauto']->content = "<table class='adminlist table table-striped table-hover' cellpadding='1' width='100%'><tbody>";
		$k = 0;
		foreach($tableField as $oneValue => $oneField){
			$tags['jeventsauto']->content .= '<tr style="cursor:pointer" onclick="insertTag(\'{JeventsAuto:'.$oneValue.'}\')" class="row'.$k.'"><td>'.$oneField.'</td></tr>';
			$k = 1-$k;
		}
		$tags['jeventsauto']->content .= '</tbody></table>';
...
<?php }
	...
	function onACYSMSDisplayFilterParams_Jevents($message){
		...
		$subscriptionStatus = array(
			'' => ' - - - ',
			'confirmed' => 'RSVP_IS_CONFIRMED',
			'pending' => 'RSVP_PENDING',
			'waiting' => 'RSVP_WAITING',
			'notwaiting' => 'RSVP_NOT_WAITING',
			'notattending' => 'RSVP_NOT_ATTENDING',
			'attending' => 'RSVP_ATTENDING',
			'maybeattending' => 'RSVP_MAYBE_ATTENDING',
			'pendingapproval' => 'RSVP_PENDING_APPROVAL');
	...
	function onACYSMSSelectData_Jevents(&$acyquery,$message){
		...
		if($eventFilter == 'events'){
			...
			foreach($eventsId as $oneEventId){
				...
				$eventConditions[] = 'jeventsattendees.at_id = '.intval($oneEventId);
		...
		if(!empty($catId)){
			$acyquery->join['jeventsattendance'] = 'JOIN #__jev_attendance jeventsattendance ON jeventsattendance.id = jeventsattendees.at_id';
	...
	function onACYSMSDailyCron(){
		...
		foreach($allMessages as $oneMessage){
			...
			if($time == 'before')	$sendingTime = time() + 86400 + (intval($oneMessage->message_receiver['auto']['jevents']['daybefore']) * 86400);
			...
			if($time == 'before')	$senddate = ACYSMS::getTime(date('Y').'-'.$sendingMonth.'-'.$sendingDay.' '.$oneMessage->message_receiver['auto']['jevents']['hour'].':'.$oneMessage->message_receiver['auto']['jevents']['min']) - (intval($oneMessage->message_receiver['auto']['jevents']['daybefore']) * 86400);
	...
	function onACYSMSReplaceTags(&$message, $send = true){
		$display = array(
					'startdate' => JText::_('JEV_EVENT_STARTDATE').': ',
					'enddate' =>  JText::_('JEV_EVENT_ENDDATE').': ',
					'eventname' => JText::_('SMS_NAME').': ',
					'location' => JText::_('JEV_EVENT_ADRESSE').': '
					);
	...
	public function onACYSMSdisplayAuthorizedFilters(&$authorizedFilters, $type){
		$newType = new stdClass();
		$newType->name = JText::_('SMS_JEVENTS');
