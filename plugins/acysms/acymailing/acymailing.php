...
class plgAcysmsAcymailing extends JPlugin
{
	...
	function plgAcysmsAcymailing(&$subject, $config){
	...
	function onACYSMSDisplayFiltersSimpleMessage($componentName, &$filters){
		...
		if($app->isAdmin() || (!$app->isAdmin() && $helperPlugin->allowSendByGroups('acymailinglist'))) $filters['acymailingList'] = $newFilter;

		$secondFilter = new stdClass();
		$secondFilter->name = JText::sprintf('SMS_INTEGRATION_FIELDS','AcyMailing');
		if($app->isAdmin() || (!$app->isAdmin() && $helperPlugin->allowSendByGroups('acymailingfield'))) $filters['acymailingField'] = $secondFilter;
	...
	function onACYSMSDisplayFilterParams_acymailingList($message){
		...
		if(!$app->isAdmin()){
			...
			foreach($frontEndFilters as $oneCondition){
				...
				$availableLists = array_merge($availableLists,$oneCondition['filterDetails']['acymailinglist']);
			}
		...
		foreach($lists as $oneList){
			if(!$app->isAdmin()){
				if(!in_array($oneList->listid,$availableLists)) continue;
			}?>
	...
	public function ajax_onAcySMSAutoMsgAdd(){
	 ...
	 function onACYSMSGetTags(&$tags) {
	 	...
	 	$tags['acymailing'] = new stdClass();
		$tags['acymailing']->name = JText::sprintf('SMS_X_USER_INFO','AcyMailing');
		$db = JFactory::getDBO();
		$tableFields = acysms_getColumns('#__acymailing_subscriber');

		$tags['acymailing']->content = '<table class="adminlist table table-striped table-hover" cellpadding="1" width="100%"><tbody>';
		$k = 0;
		foreach($tableFields as $oneField => $fieldType){
			$tags['acymailing']->content .= '<tr style="cursor:pointer" onclick="insertTag(\'{acymailing:'.$oneField.'}\')" class="row'.$k.'"><td>'.$oneField.'</td></tr>';
			$k = 1-$k;
		}
		$tags['acymailing']->content .= '</tbody></table>';
	...
	public function onACYSMSdisplayAuthorizedFilters_acymailinglist(&$authorizedFiltersSelection, $conditionNumber){
		$db = JFactory::getDBO();
		$db->setQuery('SELECT listid, name FROM #__acymailing_list');
		...
		foreach($acyLists as $oneList){
			if(!empty($frontEndFilters[$conditionNumber]['filterDetails']['acymailinglist']) && in_array($oneList->listid,$frontEndFilters[$conditionNumber]['filterDetails']['acymailinglist'])) $checked = 'checked="checked"';
			else $checked = '';
