...
{
	function plgAcysmsAcySMS(&$subject, $config){
	...
	private function _sendEmailToAdmin($user){
		...
		if(strpos($adminAddress, ',')) $recipient = explode(',', $adminAddress);
		else $recipient = array($adminAddress);

		$mailer = JFactory::getMailer();
		$mailer->isHTML(true);
		$sender = array('', $user->user_firstname.' '.$user->user_lastname);
	...
	function onACYSMSDisplayFiltersSimpleMessage($componentName, &$filters){
		...
		if($componentName != 'acysms') return;
		$db = JFactory::getDBO();
		...
		$filters['acysmsGroups'] = $newFilter;
	}
	...
	function onACYSMSDisplayFilterParams_acysmsGroups($message){
		...
		if(!$app->isAdmin()){
			...
			foreach($frontEndFilters as $oneCondition){
				...
				$availableLists = array_merge($availableLists,$oneCondition['filterDetails']['acysmsgroup']);
			}
			if(empty($availableLists)) return;

			$groupClass = ACYSMS::get('class.group');
			$allCustomerGroups = $groupClass->getFrontendGroups('group_id');
		}
		...
		foreach($groups as $oneGroup){
			if(!$app->isAdmin()){
				if($allowCustomerManagement && !in_array($oneGroup->group_id, $allCustomerGroups)) continue;
	...
	function onACYSMSSelectData_acysmsGroups(&$acyquery,$message){
		...
		if(!isset($acyquery->join['acysmsusers']) && $message->message_receiver_table != 'acysms') $acyquery->join['acysmsusers'] = 'JOIN #__acysms_users AS acysmsusers ON acysmsusers.user_joomid = joomusers.id';
	...
	public function onACYSMSdisplayAuthorizedFilters_acysmsgroup(&$authorizedFiltersSelection, $conditionNumber){
		...
		foreach($acySMSGroups as $oneGroup){
			if(!empty($frontEndFilters[$conditionNumber]['filterDetails']['acysmsgroup']) && in_array($oneGroup->group_id,$frontEndFilters[$conditionNumber]['filterDetails']['acysmsgroup'])) $checked = 'checked="checked"';
			else $checked = '';
