...
//					$params[$item[1]] = urldecode($item[2]);
		...
		foreach ($settings as $tab => $options) {
			...
			if ($tab == "action") {
				...
				foreach ($options as $idx => $option) {
					...
					if ( isset( $option['name'] ) && $option['name'] == JText::_( 'JSN_PAGEBUILDER_HELPER_SHORTCODE_MARGIN' ) && $option['id'] != 'div_margin' )
						$has_margin = 1;
	...
	public static function pbTrimWords( $text, $num_words = 55, $more = null ) {
		if ( null === $more )
			$more = JText::_( 'JSN_PAGEBUILDER_HELPER_SHORTCODE_MORE_HELLIP' );
		$original_text = $text;
