...
class com_acysmsInstallerScript {
	function install($parent) { installAcySMS(); }
	function update($parent) { installAcySMS(); }
...
class acysmsInstall{
	...
	function updatePref(){
		$this->db->setQuery("SELECT * FROM `#__acysms_config` WHERE `namekey` = 'version' LIMIT 1");
		$version = $this->db->loadObject();
		if($version->value == $this->version) return true;
		$this->update = true;
		$this->fromVersion = $version->value;
		$query = "REPLACE INTO `#__acysms_config` (`namekey`,`value`) VALUES ('version',".$this->db->Quote($this->version)."),('installcomplete','0')";
		$this->db->setQuery($query);
		$this->db->query();
	}
	function updateSQL(){
		...
		if(version_compare($this->fromVersion,'0.0.3','<')){
			$this->db->setQuery("CREATE TABLE IF NOT EXISTS `#__acysms_phone` (
			...
			$this->db->query();
			$this->db->setQuery("DROP TABLE #__acysms_receiver");
			$this->db->query();

			try{
				$this->db->setQuery("ALTER TABLE `#__acysms_statsdetails` CHANGE `statsdatails_receiver_table` `statsdetails_receiver_table` VARCHAR( 250 ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL");
				$res = $this->db->query();
			}catch(Exception $e){
				$res = null;
			}
		...
		if(version_compare($this->fromVersion,'1.0.0','<')){
			...
			$this->db->setQuery("CREATE TABLE IF NOT EXISTS `#__acysms_answer` (
			...
			try{
				$this->db->setQuery("ALTER TABLE `#__acysms_senderprofile` ADD `senderprofile_default` TINYINT NOT NULL DEFAULT 0");
				$res = $this->db->query();
			}catch(Exception $e){
				$res = null;
			}
			...
			try{
				$this->db->setQuery("ALTER TABLE `#__acysms_message` ADD `message_receiver_table` VARCHAR( 250 ) NOT NULL ");
				$res = $this->db->query();
			}catch(Exception $e){
				$res = null;
			}
			...
			try{
				$this->db->setQuery("ALTER TABLE `#__acysms_statsdetails` ADD `statsdetails_sms_id` VARCHAR( 250 ) NOT NULL , ADD `statsdetails_error` VARCHAR( 750 ) NOT NULL, ADD `statsdetails_received_date` INT UNSIGNED NOT NULL");
				$res = $this->db->query();
			}catch(Exception $e){
				$res = null;
			}
			...
			$this->db->setQuery("CREATE TABLE IF NOT EXISTS `#__acysms_user` (
		...
		if(version_compare($this->fromVersion,'1.0.1','<')){
			$this->db->setQuery("UPDATE `#__acysms_statsdetails` SET `statsdetails_sms_id` = CONCAT(`statsdetails_message_id`,'_',`statsdetails_receiver_id`) WHERE `statsdetails_sms_id` IS NULL OR `statsdetails_sms_id` = ''");
			$this->db->query();

			try{
				$this->db->setQuery("ALTER TABLE `#__acysms_statsdetails`  DROP PRIMARY KEY,   ADD PRIMARY KEY(`statsdetails_sms_id`)");
				$res = $this->db->query();
			}catch(Exception $e){
				$res = null;
			}
			...
			try{
				$this->db->setQuery("ALTER TABLE `#__acysms_statsdetails` ADD INDEX `message_index` ( `statsdetails_message_id` , `statsdetails_sentdate` )");
				$res = $this->db->query();
			}catch(Exception $e){
				$res = null;
			}
			...
			try{
				$this->db->setQuery("ALTER TABLE `#__acysms_statsdetails` ADD INDEX `receiver_index` ( `statsdetails_receiver_table` , `statsdetails_receiver_id` )");
				$res = $this->db->query();
			}catch(Exception $e){
				$res = null;
			}
		...
		if(version_compare($this->fromVersion,'1.0.3','<')){
			$this->db->setQuery("CREATE TABLE IF NOT EXISTS `#__acysms_answertrigger` (
		...
		if(version_compare($this->fromVersion,'1.1.2','<')){
			try{
				$this->db->setQuery("ALTER TABLE `#__acysms_config` CHANGE `namekey` `namekey` VARCHAR( 50 )  NOT NULL ;");
				$res = $this->db->query();
			}catch(Exception $e){
				$res = null;
			}
		...
		if(version_compare($this->fromVersion,'1.1.5','<')){
			$this->db->setQuery("CREATE TABLE IF NOT EXISTS `#__acysms_fields` (
		...
		if(version_compare($this->fromVersion,'1.2.1','<')){
			...
			$this->db->setQuery("CREATE TABLE IF NOT EXISTS `#__acysms_group` (
			...
			$this->db->setQuery("CREATE TABLE IF NOT EXISTS `#__acysms_groupuser` (
			...
			$this->db->setQuery("DELETE FROM #__acysms_phone WHERE `phone_status` = 1");
			$this->db->query();

			try{
				$this->db->setQuery("ALTER TABLE `#__acysms_phone` DROP `phone_status`");
				$res = $this->db->query();
			}catch(Exception $e){
				$res = null;
			}
		...
		if(version_compare($this->fromVersion,'1.3.1','<')){
			$this->db->setQuery("UPDATE `#__acysms_fields`
		...
		if(version_compare($this->fromVersion,'1.3.2','<')){
			try{
				$this->db->setQuery("ALTER TABLE `#__acysms_user` ADD `user_activationcode` VARCHAR( 250 ) DEFAULT NULL ;");
				$res = $this->db->query();
			}catch(Exception $e){
				$res = null;
			}
		...
		if(version_compare($this->fromVersion,'1.5.0','<')){
			$this->db->setQuery("UPDATE `#__acysms_user` SET user_phone_number = REPLACE(user_phone_number, ',0', '')");
			$this->db->query();

			$this->db->setQuery("UPDATE `#__acysms_user` SET user_phone_number = REPLACE(user_phone_number, ',', '')");
			$this->db->query();
			$this->db->setQuery("UPDATE `#__acysms_user` SET user_phone_number = REPLACE(user_phone_number, ' ', '')");
			$this->db->query();
		}
		...
		if(version_compare($this->fromVersion,'1.6.0','<')){
			try{
				$this->db->setQuery("ALTER TABLE `#__acysms_senderprofile` ADD `senderprofile_access` VARCHAR( 250 ) NOT NULL DEFAULT 'all'");
				$res = $this->db->query();
			}catch(Exception $e){
				$res = null;
			}
		...
		if(version_compare($this->fromVersion,'1.6.2','<')){
			try{
				$this->db->setQuery("ALTER TABLE `#__acysms_answertrigger` CHANGE `answertrigger_actions` `answertrigger_actions` TEXT");
				$res = $this->db->query();
			}catch(Exception $e){
				$res = null;
			}
		...
		if(version_compare($this->fromVersion,'1.7.5','<')){
			try{
				$this->db->setQuery("ALTER TABLE `#__acysms_message` ADD `message_attachment` TEXT NULL");
				$res = $this->db->query();
				$this->db->setQuery("ALTER TABLE `#__acysms_answer` ADD `answer_attachment` TEXT NULL");
				$res = $this->db->query();
			}catch(Exception $e){
				$res = null;
			}
			...
			try{
				$this->db->setQuery("ALTER TABLE `#__acysms_message` ADD `message_usecredits` TEXT NULL");
				$res = $this->db->query();
			}catch(Exception $e){
				$res = null;
			}
			...
			$this->db->setQuery("CREATE TABLE IF NOT EXISTS `#__acysms_customer` (
