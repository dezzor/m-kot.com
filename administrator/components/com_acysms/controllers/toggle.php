...
class ToggleController extends acysmsController{
	...
	function toggle(){
		...
		$task = substr($completeTask,0,strrpos($completeTask,'_'));
		$elementId = substr($completeTask,strrpos($completeTask,'_') +1);
	...
	function senderprofilesenderprofile_id($elementId,$value){
		$db	= JFactory::getDBO();
		$db->setQuery('UPDATE '.ACYSMS::table('senderprofile').' SET `senderprofile_default` = '.intval($value).' WHERE `senderprofile_id` = '.intval($elementId).' LIMIT 1');
		$db->query();
	}
