...
class FileController extends ACYSMSController{
	...
	function savecss(){
		...
		if(JFile::write($path, $csscontent)){
			ACYSMS::display(JText::_('SMS_SUCC_SAVED'),'success');
			...
			ACYSMS::display(JText::sprintf('SMS_FAIL_SAVE',$path),'error');
		}
		return $this->css();
	}
	function css(){
	...
	function _savelanguage(){
		...
		if($result){
			ACYSMS::display(JText::_('SMS_SUCC_SAVED'),'success');
			$js = "window.top.document.getElementById('image$code').src = '".ACYSMS_IMAGES."icons/icon-16-edit.png'";
			$doc = JFactory::getDocument();
			$doc->addScriptDeclaration( $js );
		...
		if(!$customresult) ACYSMS::display(JText::sprintf('SMS_FAIL_SAVE',$custompath),'error');
