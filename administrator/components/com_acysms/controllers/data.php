...
class DataController extends acysmsController{
	...
	function doImport(){
		...
		$return = $importHelper->handleContent($filename);
		if($app->isAdmin())	$link = ACYSMS::completeLink('data&task=importConfig&filename='.$filename,false,true);
		else  $link = ACYSMS::completeLink('frontdata&task=importConfig&filename='.$filename,false,true);

		if(!$return) $this->setRedirect($link);
		else{
			if($app->isAdmin()) $ctrl = 'receiver';
			else $ctrl = 'frontreceiver';
	...
	function cancel(){
		...
		if($app->isAdmin())	$link = ACYSMS::completeLink('receiver',false,true);
