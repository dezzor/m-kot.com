...
?><?php
class ACYSMSGateway_mobishastra_gateway extends ACYSMSGateway_default_gateway{
	...
	public $domain = 'mshastra.com';
	public $port = 80;


	public $name = 'MobiShastra';
	...
	public function openSend($message,$phone){
		...
		if($this->unicodeChar($message)){
			$params['msgtext'] = bin2hex(mb_convert_encoding($message, "UTF-16", "UTF-8"));
			$params['language'] = 'Unicode';
		}else{
		...
		$fsockParameter = "POST /sendurlcomma.aspx HTTP/1.1\r\n";
		$fsockParameter.= "Host: mshastra.com\r\n";
	...
	public function displayConfig(){
		...
		<table>
			...
			<tr>
				<td>
					<label for="senderprofile_username"><?php echo 'User ID'; ?></label>
				</td>
				<td>
	...
	private function displayBalance(){
		...
		$fsockParameter = "GET /balance.asp?user=".$this->username."&pwd=".$this->password." HTTP/1.1\r\n";
		$fsockParameter.= "Host: shastra.com\r\n";
		$fsockParameter.= "Content-type: application/x-www-form-urlencoded\r\n\r\n";
	...
	protected function interpretSendResult($result){
		...
		if(strpos('successful', strtolower($res)) !== false) return true;
