...
class ACYSMScountryType{
	...
	public $phonewidth = 100;
	public $phonemaxwidth = 70;
	...
	function displayPhone($value,$map){
		...
		$class = empty($this->field->fields_required) ? ' class="inputbox"' : ' class="inputbox required"';
		...
		$id = empty($this->idPhoneField) ? '' : 'id="'.htmlspecialchars($this->idPhoneField,ENT_COMPAT, 'UTF-8').'"';
		$inputphone = '<input type="text" name="'.htmlspecialchars($map,ENT_COMPAT, 'UTF-8').'[phone_num]" '.$class.' '.$id.' '.$styleline.' '.$readOnly.' value="'.htmlspecialchars($num,ENT_COMPAT, 'UTF-8').'" '.$placeholder.' />';

		return '<div style="white-space:nowrap">'.$countrycode.' '.$inputphone.'</div>';
	}
	...
	function displayCountry($value,$map,$displayCountry=1){
		...
		$divCountryCode .='
			...
			<script language="javascript" type="text/javascript">
				...
				function acysms_loadResultArray(id) {
					...
					for(i=0; i<htmlcollectionResult.length; i++) {
		...
		foreach($this->country as $code => $country){
			if(isset($this->flagPosition[$code])){
				...
				$divCountryCode .= '<tr class="acysms_countryLine" onclick="acysms_selectACountry('.$code.',\''.$country.'\','.$this->flagPosition[$code]['x'].','.$this->flagPosition[$code]['y'].','.$id.','.$displayCountry.')"><td>'.$image.'</td><td>'.$country.'</td><td style="color:#666">+'.$code.'</td></tr>';
			}
		}
