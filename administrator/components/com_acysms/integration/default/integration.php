...
class ACYSMSIntegration_default_integration{
	...
	public function ACYSMSIntegration_default_integration($moduleid = '') {
	...
	public function getQueryUsers($search, $order, $filters){
		...
		$query = 'SELECT integrationTable.*, firstname as receiver_name, receivers.email as receiver_email, integrationTable.'.$this->primaryField.' as receiver_id, integrationTable.'.$config->get('receiver_field').' as receiver_phone
	...
	function getStatDetailsQuery($filters, $order, $selectedMessage){
		...
		$query = 'SELECT stats.*, message.*, receiver.*, message.message_id as message_id, stats.statsdetails_sentdate as message_sentdate, message.message_subject as message_subject, receiver.'.$this->emailField.' as receiver_email, receiver.'.$integrationField.' as receiver_phone, stats.statsdetails_status as message_status
		...
		if(!$this->useJoomlaName)	$query .= ' JOIN '.$this->tableName.' AS receiver ON receiver.'.$this->primaryField.' = stats.statsdetails_receiver_id';
		...
		$query.= ' AND statsdetails_receiver_table = '.$db->Quote($this->componentName);
		if(!empty($order)){
				$query .= ' ORDER BY '.$order->value.' '.$order->dir;
		}
		...
		if(!$this->useJoomlaName)	$query .= ' JOIN '.$this->tableName.' AS receiver ON receiver.'.$this->primaryField.' = queue.queue_receiver_id';
		else  $query .= ' JOIN  #__users as receiver ON queue.queue_receiver_id = receiver.id';


		if(!empty($selectedMessage)) $filters[] = 'stats.statsdetails_message_id = '.intval($selectedMessage);
		if(!empty($filters))    $query.= ' WHERE ('.implode(') AND (',$filters).')';
		$query.= ' AND statsdetails_receiver_table = '.$this->componentName;
		if(!empty($order)){
				$query .= ' ORDER BY '.$order->value.' '.$order->dir;
		}
		$db->setQuery($queryCount);
	...
	function getDashboardQuery(){
		$db = JFactory::getDBO();
		$db->setQuery('SELECT integrationTable.*, integrationTable.'.$this->primaryField.' as receiver_id FROM '.$this->tableName.' as integrationTable ORDER BY integrationTable.'.$this->primaryField.' DESC LIMIT 10');
	...
	function getQuery(&$acyquery, $message){
		...
		$acyquery->where[] =  'integrationTable.enabled = 1 AND integrationTable.accept = 1 AND CHAR_LENGTH(integrationTable.`'.$config->get('receiver_field').'`) > 3 ';
		return $acyquery;
	}
	...
	function getQueueListingQuery($filters, $order){
		...
		if(!$this->useJoomlaName)	$queryCount .= ' JOIN '.$this->tableName.' AS receiver ON receiver.'.$this->primaryField.' = queue.queue_receiver_id';
	...
	function getInformationsByPhoneNumber($phoneNumber){
		...
		if(!empty($integrationPhoneField) && !empty($phoneNumberToSearch)){
			$query = 'SELECT *, '.$this->primaryField.' as receiver_id, '.$this->nameField.' as receiver_name
			FROM '.$this->tableName.'
			WHERE '.$integrationPhoneField.' = '.$db->Quote($phoneNumberToSearch).' OR '.$integrationPhoneField.' LIKE '.$db->Quote('%'.$phoneNumberToSearch);
			$db->setQuery($query);
