...
class ACYSMSIntegration_akeebasubscriptions_integration extends ACYSMSIntegration_default_integration{
	...
	public function getQueryUsers($search, $order, $filters){
		...
		$query = 'SELECT akeebasubscriptions.*, akeebasubscriptions.akeebasubs_user_id as receiver_id, joomusers.name as receiver_name, joomusers.email as receiver_email, joomuserprofile.profile_value as receiver_phone
				...
				WHERE joomuserprofile.profile_key = "akeebasubs.'.$config->get('akeebasubscriptions_field').'"';
		if(!empty($filters)){
	...
	public function getStatDetailsQuery($filters, $order, $selectedMessage){
			...
			$result = new stdClass();
			$config = ACYSMS::config();
			$integrationField = $config->get('akeebasubscriptions_field');

			if(!empty($selectedMessage)) $filters[] = 'stats.statsdetails_message_id = '.intval($selectedMessage);
			$filters[] = 'statsdetails_receiver_table = "akeebasubscriptions"';

			$query = 'SELECT stats.*, message.*, akeebasubscriptions.*, message.message_id as message_id, stats.statsdetails_sentdate as message_sentdate, message.message_subject as message_subject, joomusers.email as receiver_email, joomusers.name as receiver_name, joomuserprofile.profile_key as receiver_phone, stats.statsdetails_status as message_status
	...
	public function getDashboardQuery(){
		$db = JFactory::getDBO();
		$config = ACYSMS::config();
		$db->setQuery('SELECT akeebasubscriptions.*, akeebasubscriptions.akeebasubs_user_id as receiver_id, joomusers.email as receiver_email, joomuserprofile.profile_value as receiver_phone,
	...
	public function initQuery(&$acyquery){
		...
		$acyquery->where[] = 'joomuserprofile.profile_key = "akeebasubs.'.$config->get('akeebasubscriptions_field').'"';
	...
	public function addUsersInformations(&$queueMessage){
		...
		$query = 'SELECT akeebasubscriptions.akeebasubs_user_id as receiver_id, joomuserprofile.profile_value as receiver_phone, akeebasubscriptions.*, joomuserprofile.profile_key
				...
				JOIN '.ACYSMS::table('users',false).' AS joomusers	ON akeebasubscriptions.user_id = joomusers.id
				JOIN #__user_profiles as joomuserprofile	ON joomusers.id = joomuserprofile.user_id
				WHERE profile_key = "akeebasubs.'.$config->get('akeebasubscriptions_field').'" AND akeebasubscriptions.akeebasubs_user_id IN ('.implode(',',$userId).')';
		$db->setQuery($query);
	...
	public function getJoomUserId($userIDs = array()){
		...
		$query = 'SELECT user_id FROM #__acymailing_subscriber WHERE akeebasubs_user_id IN ('.implode(',',$userIDs).')';
		$db->setQuery($query);
		return acysms_loadResultArray($db);
	}
