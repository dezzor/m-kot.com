...
class ACYSMSIntegration_hikashop_integration extends ACYSMSIntegration_default_integration{
	...
	public function getQueryUsers($search, $order, $filters){
			...
			$searchFields = array('hikaaddress.address_firstname', 'hikaaddress.address_lastname','hikausers.user_email');
			$result = new stdClass();
			...
			$query ='SELECT hikausers.*, hikaaddress.*, hikaaddress.address_user_id as receiver_id, hikausers.user_email as receiver_email,
					 hikaaddress.'.$config->get('hikashop_field').' as receiver_phone, CONCAT_WS(" ",hikaaddress.address_firstname,hikaaddress.address_lastname) as receiver_name
	...
	public function getStatDetailsQuery($filters, $order, $selectedMessage){
		...
		$filters[] = 'statsdetails_receiver_table = "hikashop"';

		$query = 'SELECT stats.*, message.*, message.message_id as message_id, stats.statsdetails_sentdate as message_sentdate, message.message_subject as message_subject, stats.statsdetails_status as message_status, hikausers.user_email as receiver_email,
							 	hikaaddress.'.$config->get('hikashop_field').' as receiver_phone, CONCAT_WS(" ",hikaaddress.address_firstname,hikaaddress.address_lastname) as receiver_name
	...
	public function getDashboardQuery(){
			...
			$query ='SELECT hikausers.*, hikaaddress.address_user_id as receiver_id, hikausers.user_email as receiver_email,
							hikaaddress.*, joomUser.*, hikaaddress.'.$config->get('hikashop_field').' as receiver_phone,
							CONCAT_WS(" ",hikaaddress.address_firstname,hikaaddress.address_lastname) as receiver_name
							 FROM #__hikashop_address AS hikaaddress
							 LEFT JOIN #__hikashop_user as hikausers
							 ON hikaaddress.address_user_id = hikausers.user_id
							 LEFT JOIN #__users as joomUser
							 ON joomUser.id = hikausers.user_cms_id
							 WHERE hikaaddress.address_default = "1"
							LIMIT 10';
	...
	public function initQuery(&$acyquery){
			...
			$acyquery->where[] =  'CHAR_LENGTH(hikaaddress.`'.$config->get('hikashop_field').'`) > 3 AND hikaaddress.address_default = 1';
			return $acyquery;
	}
	...
	public function addUsersInformations(&$queueMessage){
		...
		$query = 'SELECT *, `'.$config->get('hikashop_field').'` as receiver_phone, CONCAT_WS(" ",address_firstname,address_lastname) as receiver_name
						FROM #__hikashop_address as hikaaddress
