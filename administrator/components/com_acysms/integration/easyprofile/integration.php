...
class ACYSMSIntegration_easyprofile_integration extends ACYSMSIntegration_default_integration{
	...
	public function getQueryUsers($search, $order, $filters){
		...
		$searchFields = array('easyProfileUsers.firstname','easyProfileUsers.lastname', 'joomusers.email');
		$result = new stdClass();
		...
		$query = 'SELECT easyProfileUsers.*, easyProfileUsers.id AS receiver_id, CONCAT_WS(" ",easyProfileUsers.firstname,easyProfileUsers.lastname) AS receiver_name, joomusers.email AS receiver_email, easyProfileUsers.'.$config->get('easyprofile_field').' AS receiver_phone
	...
	public function getStatDetailsQuery($filters, $order, $selectedMessage){
		...
		$filters[] = 'statsdetails_receiver_table = "easyprofile"';

		$query = 'SELECT stats.*, message.*, easyProfileUsers.*, message.message_id as message_id, stats.statsdetails_sentdate as message_sentdate, message.message_subject as message_subject, joomusers.email as receiver_email, easyProfileUsers.'.$config->get('easyprofile_field').' as receiver_phone, CONCAT_WS(" ",easyProfileUsers.firstname,easyProfileUsers.lastname) as receiver_name, stats.statsdetails_status as message_status
	...
	public function getDashboardQuery(){
		$db = JFactory::getDBO();
		$config = ACYSMS::config();
		$db->setQuery('SELECT easyProfileUsers.*, easyProfileUsers.id as receiver_id, CONCAT_WS(" ",easyProfileUsers.firstname,easyProfileUsers.lastname) as receiver_name, joomusers.email as receiver_email, easyProfileUsers.'.$config->get('easyprofile_field').' as receiver_phone
					FROM #__jsn_users AS easyProfileUsers
					JOIN #__users AS joomusers
					ON easyProfileUsers.id = joomusers.id
					ORDER BY easyProfileUsers.id DESC
					LIMIT 10');
	...
	public function initQuery(&$acyquery){
		...
		$acyquery->where[] = ' CHAR_LENGTH(easyProfileUsers.`'.$config->get('easyprofile_field').'`) > 3';
		return $acyquery;
	}
	...
	public function addUsersInformations(&$queueMessage){
		...
		$query = 'SELECT *, '.$config->get('easyprofile_field').' AS receiver_phone, CONCAT_WS(" ", firstname, lastname) AS receiver_name, email AS receiver_email
