...
class ACYSMSIntegration_seblod_integration extends ACYSMSIntegration_default_integration{
	...
	public function getQueryUsers($search, $order, $filters){
		...
		$query = 'SELECT joomusers.*, joomusers.id AS receiver_id, joomusers.name AS receiver_name, joomusers.email AS receiver_email, seblodusers.'.$config->get('seblod_field').' AS receiver_phone
	...
	public function getStatDetailsQuery($filters, $order, $selectedMessage){
			...
			$filters[] = 'statsdetails_receiver_table = "seblod"';

			$query = 'SELECT stats.*, message.*, joomusers.*, message.message_id as message_id, stats.statsdetails_sentdate as message_sentdate, message.message_subject as message_subject, joomusers.name AS receiver_name, joomusers.email AS receiver_email, seblodusers.'.$integrationField.' AS receiver_phone, stats.statsdetails_status as message_status
	...
	public function getDashboardQuery(){
		$db = JFactory::getDBO();
		$config = ACYSMS::config();
		$db->setQuery('SELECT joomusers.*, joomusers.id AS receiver_id, joomusers.name AS receiver_name, joomusers.email AS receiver_email, seblodusers.'.$config->get('seblod_field').' AS receiver_phone
						FROM '.ACYSMS::table('users',false).' AS joomusers
						JOIN #__cck_store_item_users AS seblodusers ON joomusers.id = seblodusers.id
						ORDER BY joomusers.id DESC
					 	LIMIT 10');
	...
	public function initQuery(&$acyquery){
		...
		$acyquery->where[] = 'joomusers.block=0 AND CHAR_LENGTH(seblodusers.'.$config->get('seblod_field').') > 3';
		return $acyquery;
	}
	...
	public function addUsersInformations(&$queueMessage){
		...
		$query = 'SELECT joomusers.*, seblodusers.*, seblodcckstoreform.*, joomusers.id AS receiver_id, joomusers.name AS receiver_name, joomusers.email AS receiver_email, seblodusers.'.$config->get('seblod_field').' AS receiver_phone
				FROM '.ACYSMS::table('users',false).' AS joomusers
				JOIN #__cck_store_item_users AS seblodusers ON joomusers.id = seblodusers.id
	...
	public function getQueueListingQuery($filters, $order){
		...
		$query = 'SELECT queue.*, queue.queue_priority as queue_priority, queue.queue_try as queue_try, queue.queue_senddate as queue_senddate, message.message_subject as message_subject, joomusers.id as receiver_id, joomusers.name as receiver_name, seblodusers.'.$config->get('seblod_field').' AS receiver_phone
	...
	public function getInformationsByPhoneNumber($phoneNumber){
		...
		if(!empty($integrationPhoneField)){
			$db->setQuery('SELECT joomusers.id as receiver_id, joomusers.name as receiver_name, seblodusers.'.$config->get('seblod_field').' AS receiver_phone
				FROM '.ACYSMS::table('users',false).' AS joomusers
				JOIN #__cck_store_item_users AS seblodusers ON joomusers.id = seblodusers.id
				WHERE  seblodusers.'.$config->get('seblod_field').' = '.$db->Quote($phoneNumberToSearch).' OR seblodusers.'.$config->get('seblod_field').' LIKE '.$db->Quote('%'.$phoneNumberToSearch));
