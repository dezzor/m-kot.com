...
class ACYSMSIntegration_virtuemart_2_integration extends ACYSMSIntegration_default_integration{
	...
	public function getQueryUsers($search, $order, $filters){
		...
		$searchFields = array('virtuemartusers_2.first_name','virtuemartusers_2.last_name','joomusers.email');
		$result = new stdClass();
		...
		if(!empty($filters)){
			$queryCount .= ' LEFT JOIN '.ACYSMS::table('users',false).' as joomusers ON virtuemartusers_2.virtuemart_userinfo_id = joomusers.id';
	...
	public function getStatDetailsQuery($filters, $order, $selectedMessage){
		...
		$filters[] = 'statsdetails_receiver_table = "virtuemart_2"';

		$query = 'SELECT stats.*, message.*, message.message_id as message_id, stats.statsdetails_sentdate as message_sentdate, message.message_subject as message_subject, CONCAT_WS(" ",virtuemartusers_2.first_name,virtuemartusers_2.last_name) as receiver_name, joomusers.email as receiver_email, virtuemartusers_2.'.$config->get('virtuemart_2_field').' as receiver_phone, stats.statsdetails_status as message_status
				...
				LEFT JOIN '.ACYSMS::table('users',false).' AS joomusers ON virtuemartusers_2.virtuemart_user_id = joomusers.id ';
	...
	public function getDashboardQuery(){
		$config = ACYSMS::config();
		$db = JFactory::getDBO();
		$db->setQuery('SELECT virtuemartusers_2.*, virtuemartusers_2.virtuemart_userinfo_id as receiver_id, joomusers.email as receiver_email, CONCAT_WS(" ",virtuemartusers_2.first_name,virtuemartusers_2.last_name) as receiver_name, virtuemartusers_2.'.$config->get('virtuemart_2_field').' as receiver_phone
				FROM #__virtuemart_userinfos as virtuemartusers_2
				LEFT JOIN '.ACYSMS::table('users',false).' AS joomusers
				ON virtuemartusers_2.virtuemart_user_id = joomusers.id
				ORDER BY virtuemartusers_2.virtuemart_userinfo_id DESC LIMIT 10');
