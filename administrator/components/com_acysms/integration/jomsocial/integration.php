...
class ACYSMSIntegration_jomsocial_integration extends ACYSMSIntegration_default_integration{
	...
	public function getQueryUsers($search, $order, $filters){
		...
		$queryCount = 'SELECT jomsocialusers.*, jomsocialusers.userid as receiver_id, joomusers.name as receiver_name, joomusers.email as receiver_email, fieldsvalue.value as receiver_phone
				FROM #__community_users as jomsocialusers
				JOIN #__users as joomusers ON jomsocialusers.userid = joomusers.id
				LEFT JOIN #__community_fields_values fieldsvalue ON fieldsvalue.user_id = jomsocialusers.userid';
		if(!empty($filters)){
	...
	function getStatDetailsQuery($filters, $order, $selectedMessage){
		...
		$filters[] = 'statsdetails_receiver_table = "jomsocial"';

		$query = 'SELECT stats.*, message.*, jomsocialusers.*, message.message_id as message_id, stats.statsdetails_sentdate as message_sentdate, message.message_subject as message_subject, joomusers.name as receiver_name, joomusers.email as receiver_email, fieldsvalue.value as receiver_phone, stats.statsdetails_status as message_status
	...
	function getDashboardQuery(){
		...
		$db->setQuery('SELECT jomsocialusers.*, jomsocialusers.userid as receiver_id, joomusers.name as receiver_name, joomusers.email as receiver_email, fieldsvalue.value as receiver_phone
				FROM #__community_users as jomsocialusers
				JOIN #__users as joomusers ON jomsocialusers.userid = joomusers.id
				JOIN #__community_fields_values fieldsvalue ON fieldsvalue.user_id = jomsocialusers.userid
				WHERE fieldsvalue.field_id = '.$config->get('jomsocial_field').' ORDER BY jomsocialusers.userid DESC LIMIT 10');
	...
	function initQuery(&$acyquery){
		...
		$acyquery->where[] =  'fieldsvalue.field_id = '.$config->get('jomsocial_field').' AND joomusers.block=0 AND CHAR_LENGTH(fieldsvalue.value) > 3';
		return $acyquery;
	}
	...
	function addUsersInformations(&$queueMessage){
		...
		$queryPhone = 'SELECT value as receiver_phone, user_id FROM #__community_fields_values WHERE field_id = '.$db->Quote($config->get('jomsocial_field')).' AND user_id IN ('.implode(',',$userId).')';
	...
	function getQueueListingQuery($filters, $order){
		...
		$query .= ' WHERE fieldsvalue.field_id = '.$config->get('jomsocial_field');
		...
		$queryCount .= ' WHERE fieldsvalue.field_id = '.$config->get('jomsocial_field');
