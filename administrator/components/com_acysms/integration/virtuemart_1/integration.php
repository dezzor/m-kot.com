...
class ACYSMSIntegration_virtuemart_1_integration extends ACYSMSIntegration_default_integration{
	...
	public function getQueryUsers($search, $order, $filters){
		...
		$searchFields = array('virtuemartusers_1.first_name','virtuemartusers_1.last_name','joomusers.email');
		$result = new stdClass();
	...
	function getDashboardQuery(){
		$config = ACYSMS::config();
		$db = JFactory::getDBO();
		$db->setQuery('SELECT virtuemartusers_1.*, virtuemartusers_1.user_id as receiver_id, joomusers.email as receiver_email, CONCAT_WS(" ",virtuemartusers_1.first_name,virtuemartusers_1.last_name) as receiver_name, virtuemartusers_1.'.$config->get('virtuemart_1_field').' as receiver_phone
				FROM #__vm_user_info as virtuemartusers_1
				LEFT JOIN '.ACYSMS::table('users',false).' AS joomusers
				ON virtuemartusers_1.user_id = joomusers.id
				ORDER BY virtuemartusers_1.user_id DESC LIMIT 10');
