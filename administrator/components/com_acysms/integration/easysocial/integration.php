...
class ACYSMSIntegration_easysocial_integration extends ACYSMSIntegration_default_integration{
	...
	public function ACYSMSIntegration_easysocial_integration($moduleid = '') {
	 ...
	 public function getPhoneField(){
		...
		foreach($profilesList as $oneProfile) {
			$fieldList = array();
			foreach($phonefieldList as $onePhoneField) {
				if ($onePhoneField->profile == $oneProfile->profile)
				$fieldList[] = JHTML::_('select.option', JText::_($onePhoneField->column), JText::_($onePhoneField->name),'column','name');
			}
	...
	function getStatDetailsQuery($filters, $order, $selectedMessage){
		...
		$filters[] = 'statsdetails_receiver_table = "'.$this->componentName.'"';
		$filters[] = 'socialprofiles.id = '.intval($this->submoduleId);

		$query = 'SELECT stats.*, message.*, easysocialusers.*, message.message_id as message_id, stats.statsdetails_sentdate as message_sentdate, message.message_subject as message_subject, joomusers.name as receiver_name, joomusers.email as receiver_email, fieldsvalue.data as receiver_phone, stats.statsdetails_status as message_status
	...
	function getDashboardQuery(){
		...
		$db->setQuery('SELECT easysocialusers.*, easysocialusers.user_id as receiver_id, joomusers.name as receiver_name, joomusers.email as receiver_email, fieldsvalue.data as receiver_phone
				FROM #__social_users as easysocialusers
				JOIN #__users as joomusers ON easysocialusers.user_id = joomusers.id
				...
				ORDER BY easysocialusers.user_id DESC
				LIMIT 10');
