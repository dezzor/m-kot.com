...
?><div id="acysms_content" >
	...
	<form action="<?php echo JRoute::_('index.php?option=com_acysms&ctrl='.JRequest::getCmd('ctrl')); ?>" method="post" name="adminForm"  id="adminForm" autocomplete="off">
		<fieldset class="adminform" width="100%" id="finalSend">
			<legend><?php echo JText::_( 'SMS_SEND' ); ?></legend>
		...
		if((!$this->app->isAdmin() && $this->allowCustomerManagement && ($this->credits <= 0 || ($this->numberUser * $this->partInformations->nbParts) > $this->credits)) || $this->app->isAdmin() && empty($this->numberUser) || !empty($this->alreadyInQueue)){
			?>
				<div>
					<button class="btn" type="submit" onclick="<?php  if(ACYSMS_J30) echo "Joomla.submitbutton('preview')"; else echo "submitbutton('preview')"; ?> "><?php echo JText::_('SMS_CANCEL')?></button>
				</div>
			<?php
		}else{
			?>
				<div>
					<button class="btn" type="submit" onclick="<?php  if(ACYSMS_J30) echo "Joomla.submitbutton('send')"; else echo "submitbutton('send')"; ?> "><?php echo JText::_('SMS_SEND')?></button>
				</div>
			<?php
		}
		?>
