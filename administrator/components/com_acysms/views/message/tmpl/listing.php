				...
				<tr class="<?php echo "row$k"; ?>">
					...
					<td align="center">
						...
						if(ACYSMS::isAllowed($this->config->get('acl_stats_manage','all'))){
							...
							<a class="modal"  href="<?php echo $urlStat; ?>" rel="{handler: 'iframe', size: {x: 800, y: 590}}"><img src="<?php echo ACYSMS_IMAGES;?>stats.png" alt="<?php echo JText::_('SMS_STATS',true) ?>" /></a>
						<?php
						}
						$ctrl = JRequest::getCmd('ctrl');
						if(ACYSMS::isAllowed($this->config->get('acl_messages_create_edit','all'))) echo '<a href="'.ACYSMS::completeLink($ctrl.'&task=edit&cid='.$row->message_id.$this->itemId).'">'.ACYSMS::dispSearch($row->message_subject, $this->pageInfo->search).'</a>';
