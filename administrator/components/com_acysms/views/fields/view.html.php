...
class FieldsViewFields extends acysmsView
{
	...
	function form(){
		...
		if(!empty($field->fields_fieldid)) $fieldTitle = ' : '.$field->fields_namekey;
		else $fieldTitle = '';
		ACYSMS::setTitle(JText::_('SMS_FIELD').$fieldTitle,'fields','fields&task=edit&fields_fieldid='.$fieldid);
	...
	function listing(){
		...
		JToolBarHelper::deleteList(JText::_('SMS_VALIDDELETEITEMS'));
		JToolBarHelper::divider();
		$bar->appendButton( 'Pophelp','customfields');
		...
		ACYSMS::setTitle(JText::_('SMS_EXTRA_FIELDS'),'fields','fields');
