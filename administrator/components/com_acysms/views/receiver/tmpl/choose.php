...
<form action="index.php?option=<?php echo ACYSMS_COMPONENT ?>&amp;ctrl=receiver" method="post" name="adminForm" id="adminForm" >
	<table>
		<tr>
			<td width="100%" id="receiverfilter">
				...
				<button class="btn" onclick="document.getElementById('search').value='';this.form.submit();"><?php echo JText::_( 'SMS_RESET' ); ?></button>
			</td>
			<td nowrap="nowrap">
			</td>
		</tr>
	...
	<table class="adminlist table table-striped table-hover" cellpadding="1">
		...
		for ($i = 0, $a = count($this->rows); $i < $a; $i++) {
			...
			$onClick = '';
			if($this->jsFct == 'affectTestUser') $onClick = 'onclick="window.top.affectTestUser(\''.strip_tags(intval($row->receiver_id)).'\',\''.addslashes(strip_tags($row->receiver_name)).'\',\''.addslashes(strip_tags($phoneHelper->getValidNum($row->receiver_phone))).'\',\''.addslashes(strip_tags($this->htmlID)).'\'); acysms_js.closeBox(true);"';
				...
				<tr class="<?php echo "row$k"; ?>" style="cursor:pointer" <?php echo $onClick; ?>>
					...
					<td align="center">
						...
						if(!empty ($row->receiver_phone)) {
							...
							if(!($phone))	echo '<font color="red" >' . $row->receiver_phone . '</font>';
							else	echo $phone;
						} else
							echo "";
						?>
