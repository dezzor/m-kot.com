...
class CustomerViewCustomer extends acysmsView
{
	...
	function listing(){
		...
		if($app->isAdmin()){
			ACYSMS::setTitle(JText::_('SMS_CUSTOMERS'), $this->icon,'customer');
			...
			$bar->appendButton( 'Pophelp','customer');
	...
	function form(){
		...
		ACYSMS::setTitle(JText::_('SMS_CUSTOMERS'),'smscustomer',$this->ctrl.'&task=edit&customer_id='.$customerID);
