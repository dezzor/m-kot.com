...
class userViewuser extends acysmsView
{
	...
	function form(){
		...
		if(!empty($userId)){
			...
			$subscription = $userClass->getSubscription($userId);
		...
		if($app->isAdmin())	$extraFields = $fieldsClass->getFields('backend',$user);
		else $extraFields = $fieldsClass->getFields('frontcomp',$user);


		if($app->isAdmin()){
				ACYSMS::setTitle(JText::_('SMS_USER'),'smsusers',$this->ctrl.'&task=edit&user_id='.$userId);
	...
	public function choosejoomuser(){
		...
		}else	$pageInfo->elements->total = $queryUser->count;
