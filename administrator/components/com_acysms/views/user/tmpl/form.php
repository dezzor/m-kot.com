...
<form action="<?php echo JRoute::_('index.php?option=com_acysms&ctrl='.JRequest::getCmd('ctrl')); ?>" method="post" name="adminForm"  id="adminForm" enctype="multipart/form-data">
		...
		<tr>
			<td valign="top">
				...
				if(!empty($this->coreFields)){
					include(dirname(__FILE__).DS.'corefields.'.basename(__FILE__));
				}
				?>
			</td>
			<td valign="top">
			...
			<table class="adminlist table table-striped table-hover" cellspacing="1" align="center">
				<thead>
					<tr>
						...
						<th  class="title" nowrap="nowrap">
						<?php echo JText::_( 'SMS_STATUS' ); echo '<span style="display:inline-block;font-style:italic;margin-left:50px">'.$this->filters->statusquick.'</span>';?>
						</th>
	...
	<?php $app = JFactory::getApplication(); if($app->isAdmin()) echo '<input type="hidden" id="user_joomid" name="user_joomid" value="'.$this->escape(@$this->user->user_joomid).'" />';?>
