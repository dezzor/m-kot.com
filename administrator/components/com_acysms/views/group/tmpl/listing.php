...
?><div id="acysms_content">
	...
	<form action="index.php?option=<?php echo ACYSMS_COMPONENT ?>&amp;ctrl=group" method="post" name="adminForm" id="adminForm" >
		<table>
			<tr>
				<td width="100%">
					<input placeholder="<?php echo $this->escape(JText::_('SMS_SEARCH')); ?>" type="text" name="search" id="search" value="<?php echo $this->escape($this->pageInfo->search);?>" class="text_area" />
					<button class="btn" onclick="document.adminForm.limitstart.value=0;this.form.submit();"><?php echo JText::_( 'SMS_GO' ); ?></button>
					<button class="btn" onclick="document.adminForm.limitstart.value=0;document.getElementById('search').value='';this.form.submit();"><?php echo JText::_( 'SMS_RESET' ); ?></button>
				</td>
				<td nowrap="nowrap">
			...
			<thead>
				<tr>
					...
					<th class="title titletoggle">
						<?php echo JHTML::_('grid.sort',   JText::_('SMS_ENABLED'), 'acysmsgroup.group_published', $this->pageInfo->filter->order->dir, $this->pageInfo->filter->order->value ); ?>
					</th>
			...
			<tbody>
				...
				?>
					<tr class="<?php echo "row$k"; ?>">
						...
						<td align="center">
						<?php
							echo ACYSMS::tooltip($row->group_description, $row->group_name, 'tooltip.png', $row->group_name, ACYSMS::completeLink('group&task=edit&group_id='.$row->group_id));
						?>
						</td>
						<td align="center">
							<?php echo $row->nbsub; ?>
						...
						<td align="center">
							<span id="<?php echo $publishedid ?>" class="spanloading"><?php echo $this->toggleClass->toggle($publishedid,$row->group_published, 'group') ?></span>
						</td>
						<td align="center">
