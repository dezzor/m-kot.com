...
class ACYSMSupdateHelper{
	...
	function installDefaultSenderProfile(){
		...
		$this->db->setQuery("INSERT INTO #__acysms_senderprofile (senderprofile_id, senderprofile_userid, senderprofile_name, senderprofile_gateway, senderprofile_params) VALUES ('1','".$user->id."', 'test', 'test', '')");
		$this->db->query();
	}
	...
	public function installDefaultAnswerTrigger(){
		...
		$answerTrigger->answertrigger_actions = serialize(array('selected' => 'unsubscribe'));
	...
	function addUpdateSite(){
		...
		$object->location='http://www.acyba.com/component/updateme/updatexml/component-acysms/version-'.$config->get('version').'/level-pro/li-'.urlencode(base64_encode(ACYSMS_LIVE)).'/file-extension.xml';
		$object->enabled=1;
