...
class ACYSMSqueueHelper{
	...
	public function process(){
		...
		if($this->report){
			...
			$disp .= '<br /><br />';
			$url = JURI::base().'index.php?option=com_acysms&ctrl=message&tmpl=component&task=processQueue&message_id='.$this->message_id.'&totalsend='.$this->total.'&alreadysent=';
		...
		foreach($queueElements as $oneQueue){
			...
			if(!isset($this->senderProfiles[$myMessage->message_senderprofile_id])){
				...
				if(!$gateway){
					...
					$app->enqueueMessage('Could not load the sender profile with the ID : '.$myMessage->message_senderprofile_id.'. Please edit the message '.$myMessage->message_subject.' and select the sender profile','notice');
					continue;
				}
			...
			if(!empty($myMessage->message_usecredits)){
				...
				if($nbCreditsLeft[$myMessage->message_senderid] - $this->nbParts <= 0){
				...
				$nbCreditsLeft[$myMessage->message_senderid] --;
			}
			...
			$myresult->smsid = $oneQueue->queue_receiver_id.'_'.$myMessage->message_id.'_'.time();

			if(!$sendit){
				$this->errorSend ++;
				$this->queueDelete[$oneQueue->queue_message_id][$oneQueue->queue_receiver_table][] = $oneQueue->queue_receiver_id;
				if($blockedUser)	$this->statsAdd[$oneQueue->queue_message_id][$oneQueue->queue_receiver_table][-3][] = array('smsid'=>$myresult->smsid, 'receiverid'=>$oneQueue->queue_receiver_id,'errors'=>implode(' | ',$this->senderProfiles[$myMessage->message_senderprofile_id]->errors));
	...
	private function closeAllConnections(){
		...
		foreach($this->openSend as $oneConnection){
			...
			if($result){
				...
				$messageOnScreen .= JText::sprintf('SMS_SUCC_SENT','<b><i>'.$oneConnection->message->message_subject.'</i></b>','<b><i>'.$oneConnection->queue->receiver_phone.'</i></b>');
	...
	private function _handleErrors($oneConnection){
		...
		if(empty($maxTry) || $oneConnection->queue->queue_try < $maxTry-1){
			$messageOnScreen .= ' => '.JText::_('SMS_QUEUE_NEXT_TRY');
