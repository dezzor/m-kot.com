...
class ACYSMSimportHelper{
	...
	function textarea(){
		...
		if(!JFile::write($uploadPath.DS.$this->filename,$content)){
			$app->enqueueMessage('Could not create the file '.$uploadPath.$this->filename,'error');
			return false;
		}
	...
	function file(){
		...
		if(empty($importFile['name'])){
			$app->enqueueMessage(JText::_('SMS_BROWSE_FILE'),'notice');
			return false;
		}
		...
		if(!JFile::upload($importFile['tmp_name'], $uploadPath.$this->filename)){
			if( !move_uploaded_file($importFile['tmp_name'], $uploadPath . $this->filename)) {
				$app->enqueueMessage(JText::sprintf( 'SMS_FAIL_UPLOAD','<b><i>'.$importFile['tmp_name'].'</i></b>','<b><i>'.$uploadPath.$this->filename.'</i></b>'), 'error');
				return false;
			}
		}
	...
	function handleContent($filename){
		...
		while (isset($importLines[$i])) {
			...
			if(count($data) != $numberColumns){
				...
				static $errorcount = 0;
				if(empty($errorcount)){
					$app->enqueueMessage(JText::sprintf('SMS_IMPORT_ARGUMENTS',$numberColumns),'error');
				}
				$errorcount++;
				if($errorcount<20){
					$app->enqueueMessage(JText::sprintf('SMS_IMPORT_ERRORLINE','<b><i>'.$importLines[$i-1].'</i></b>'),'notice');
			...
			if(!$phoneNumberField && !$specifyFields){
				$app->enqueueMessage(JText::_('SMS_IMPORT_PHONE_NUMBER'),'warning');
				return false;
			}
			if($specifyFields){
				$app->enqueueMessage(JText::_('SMS_PLEASE_SPECIFY_FIELDS'),'warning');
				return false;
			}
			$newUser->user_phone_number = trim(str_replace(array(' ',"\t"),'',$encodingHelper->change($newUser->user_phone_number,'UTF-8','ISO-8859-1')));
			...
			if(!$phoneHelper->getValidNum($newUser->user_phone_number)){
				...
				if($errorcountfail<50){
					$app->enqueueMessage(JText::sprintf('SMS_NOT_VALID_PHONE_NUMBER','<b><i>'.$newUser->user_phone_number.'</i></b>').' | '.($i-1).' : '.$importLines[$i-1],'notice');
			...
			unset($newUser->user_id); unset($newUser->user_joomid);
		...
		if($this->dispresults){
			$app->enqueueMessage(JText::sprintf('SMS_IMPORT_REPORT',$this->totalTry,$this->totalInserted,$this->totalTry - $this->totalValid,$this->totalValid - $this->totalInserted));
		}
	...
	function _insertUsers($users){
		...
		$columns = reset($users);
		$query = $this->overwriteExisting ? 'REPLACE' : 'INSERT IGNORE';
		$query .= ' INTO '.ACYSMS::table('user').' (`'.implode('`,`',array_keys(get_object_vars($columns))).'`) VALUES (';
		foreach($users as $a => $oneUser){
			$value = array();
	...
	public function get10Lines($filename){
		...
		if(!$contentFile){
			$app->enqueueMessage(JText::sprintf( 'SMS_FAIL_OPEN','<b><i>'.$uploadPath.$filename.'</i></b>'), 'error');
			return false;
		};
