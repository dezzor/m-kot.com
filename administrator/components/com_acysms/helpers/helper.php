...
if($app->isAdmin()){
	...
	define('ACYSMS_LIVE',rtrim(JURI::root(),'/').'/');
...
else{ echo 'AcySMS: Could not load compat file for J'.JVERSION;return;}
...
class ACYSMS{
	...
	static function listingSearch($escapedSearch){
		$app = JFactory::getApplication();
		if(ACYSMS_J30 && $app->isAdmin()){
			?>
				<div class="filter-search btn-group pull-left">
					<label for="search" class="element-invisible"><?php echo JText::_('SMS_SEARCH');?></label>
					<input type="text" name="search" id="search" value="<?php echo $escapedSearch;?>" class="text_area" placeholder="<?php echo JText::_('SMS_SEARCH'); ?>" title="<?php echo JText::_('SMS_SEARCH'); ?>" />
				</div>
				<div class="btn-group pull-left hidden-phone">
					<button onclick="document.adminForm.limitstart.value=0;this.form.submit();" class="btn tip hasTooltip" type="submit" title="<?php echo JText::_('SMS_SEARCH'); ?>" ><i class="icon-search"></i></button>
					<button onclick="document.adminForm.limitstart.value=0;document.getElementById('search').value='';this.form.submit();" class="btn tip hasTooltip" type="button" title="<?php echo JText::_( 'SMS_RESET' ); ?>" ><i class="icon-remove"></i></button>
				</div>
			<?php
		}else{
			?>
				<input placeholder="<?php echo JText::_('SMS_SEARCH'); ?>" type="text" name="search" id="search" value="<?php echo $escapedSearch;?>" class="text_area" />
				<button class="btn" onclick="document.adminForm.limitstart.value=0;document.adminForm.task.value='';this.form.submit();"><?php echo JText::_( 'SMS_GO' ); ?></button>
				<button class="btn" onclick="document.adminForm.limitstart.value=0;document.adminForm.task.value='';document.getElementById('search').value='';this.form.submit();"><?php echo JText::_( 'SMS_RESET' ); ?></button>
			<?php
		}
	}
	...
	static public function createDir($dir,$report = true){
		...
		if(!JFolder::create($dir)){
			if($report) $this->display('Could not create the directory '.$dir,'error');
			return false;
		}
		if(!JFile::write($dir.DS.'index.html',$indexhtml)){
			if($report) $this->display('Could not create the file '.$dir.DS.'index.html','error');
		}
	...
	static public function tooltip($desc,$title=' ', $image='tooltip.png', $name = '',$href='', $link=1){
