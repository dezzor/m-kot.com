...
class ACYSMSorderHelper{
	...
	function order($down = true){
		...
		if($down)$secondElement->$orderingColumnName--;
		...
		if($status){
			...
			$app->enqueueMessage(JText::_( 'SMS_SUCC_MOVED' ), 'message');
		}
		return $status;
	}
	function save(){
		...
		if($status){
			$app->enqueueMessage(JText::_( 'SMS_NEW_ORDERING_SAVED' ), 'message');
		}else{
			$app->enqueueMessage(JText::_( 'SMS_ERROR_ORDERING' ), 'error');
		}
		return $status;
	}
	function reOrder(){
