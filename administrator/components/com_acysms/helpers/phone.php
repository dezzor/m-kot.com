...
class ACYSMSphoneHelper{
	...
	function sendVerificationCode($phoneNumber, $activationType = 'activation_optin'){

		$db = JFactory::getDBO();
		$config = ACYSMS::config();
		$user = $this->_phoneNumberVerification($phoneNumber);
	...
	function verifyActivation($phoneNumber, $activationCode, $activationType = 'activation_optin', $removeActivationCode = 'true'){
		...
		if(empty($user->user_activationcode[$activationType])){
			$this->errorCode = 'alreadyActivated';
			$this->error = JText::_('SMS_USER_ALREADY_ACTIVATED');
			return false;
		}
	...
	private function _phoneNumberVerification($phoneNumber){
		$validPhone = $this->getValidNum($phoneNumber);
		if(!$validPhone){
			$this->errorCode = 'validPhoneNumber';
			$this->error = JText::sprintf('SMS_INVALID_PHONE_NUMBER', $phoneNumber);
			return false;
		}
		...
		if(empty($user)){
			$this->errorCode = 'unknownPhoneNumber';
			$this->error = JText::sprintf('SMS_UNKNOWN_PHONE_NUMBER', $phoneNumber);
			return false;
		}
	...
	public function displayMMS($message,$isForm) {
		if(!$isForm) {
			...
			if(!$senderProfileClass->getGateway($message->message->message_senderprofile_id)->handleMMS)	return;
		}
		$html = '
		<script>
