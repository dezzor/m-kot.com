...
class ACYSMSqueueClass extends ACYSMSClass{
	...
	function getReady($limit,$message_id = 0){
		...
		foreach($results as $oneResult){
			if(empty($oneResult->queue_paramqueue))	$oneResult->queue_paramqueue = new stdClass();
	...
	function getScheduled(){
		$db = JFactory::getDBO();
		$db->setQuery('SELECT * FROM '.ACYSMS::table('message').' WHERE message_senddate > 0 AND message_status = "scheduled" ORDER BY  message_senddate ASC ');
		return $db->loadObjectList();
	}

	private function getReadyMessage(){
		$db = JFactory::getDBO();
		$db->setQuery('SELECT message_id, message_senddate, message_subject FROM '.ACYSMS::table('message').' WHERE message_senddate > 0 AND message_status="scheduled" AND message_senddate <= '.(time()+1200).' ORDER BY message_senddate ASC');
