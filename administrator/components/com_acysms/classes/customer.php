...
class ACYSMScustomerClass extends ACYSMSClass{
	...
	function saveForm(){
		...
		if($customer->customer_senderprofile_id == 0){
			$senderProfileClass = ACYSMS::get('class.senderprofile');
			$defaultSender = $senderProfileClass->getDefaultSenderProfile();
			$customer->customer_senderprofile_id == $defaultSender->senderprofile_id;
		}
	...
	function sendLowCreditsNotification($user_id){
		...
		if($send !== true){
			...
			$cronHelper->messages = array('Error while sending the low credits nofication');
			$cronHelper->detailMessages = array($send->__toString());
	...
	public function changeCredits($joomlaUserId, $nbCredits, $action = 'set'){
		...
		if($action == 'add')	$customer->customer_credits += intval($nbCredits);
		else if($action == 'set') $customer->customer_credits = intval($nbCredits);
		else return;
