...
class plgAcysmsBirthdaysms extends JPlugin
{
	...
	function onAcySMSTestPlugin(){
	 ...
	 function onACYSMSDailyCron(){
		...
		foreach($allMessages as $oneMessage){
			...
			if($time == 'before')	$birthday = time() + 86400 + (intval($oneMessage->message_receiver['auto']['birthday']['daybefore']) * 86400);
			...
			if($time == 'before')	$senddate = ACYSMS::getTime(date('Y').'-'.$birthdayMonth.'-'.$birthdayDay.' '.$oneMessage->message_receiver['auto']['birthday']['hour'].':'.$oneMessage->message_receiver['auto']['birthday']['min']) - (intval($oneMessage->message_receiver['auto']['birthday']['daybefore']) * 86400);
	...
	function ajax_getbirthdayfields(){
		...
		switch ($table) {
			case 'acymailing':
					$query = 'SELECT  namekey AS id, fieldname AS name FROM #__acymailing_fields WHERE type = "birthday"';
					break;
			case 'communitybuilder':
			...
			case 'virtuemart_1':
					...
					break;
			case 'acysms':
					$query = 'SELECT  fields_namekey AS id, fields_namekey AS name FROM #__acysms_fields WHERE fields_type = "birthday"';
					break;
		}
		if(!empty($query)){
			$db->setQuery($query);
