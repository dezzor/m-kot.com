...
class plgAcysmsRsevent extends JPlugin {
	...
	function plgAcysmsRsevent(& $subject, $config) {
	...
	?>
		...
		function addEventTag(contentid,rowClass)
		{
			...
			for(var i in selectedContents)
			{
				...
				{
					if(wrapNumber)
						tag = tag + "{" + "<?php echo $tagName; ?>:"+wrapNumber+ "| "+"<?php echo $nextRsEventsParameter; ?>:"+selectedContents[i]+otherinfo+"}";
					else
...
public function onACYSMSchooseCategoryEvents_RsEventsPro() {
		...
		<div id="acysms_content">
				...
				for ($i = 0, $a = count($rows); $i < $a; $i++) {
					$row = $rows[$i];
						if($context == 'filters') $onClick = 'addSelectedCategory('.$row->id.',\''.$row->title.'\',\''.$idToUpdate.'\');';
	...
	function onACYSMSGetTags(&$tags) {
		...
		$tags['rsevent'] = new stdClass();
		$tags['rsevent']->name = JText::_('SMS_RSEVENTSPRO');

		$nbEvent = array ();
		for ($i = 1; $i <= 25; $i += 1) {
			$nbEvent[] = JHTML::_('select.option', $i, $i);
		}
		$nbEventSelection = JHTML::_('select.genericlist', $nbEvent, "name", "", 'value', 'text', 'all', 'nbEvent_RsEventsPro');
		...
		$eventSelection = '<div>'.JText::_('SMS_SELECT_EVENT').' <a class="modal"  onclick="window.acysms_js.openBox(this,\'index.php?option=com_acysms&tmpl=component&ctrl='.$ctrl.'&task=tag&fctplug=chooseEvents_RsEventsPro\');return false;" rel="{handler: \'iframe\', size: {x: 800, y: 500}}"><img class="icon16" src="'.ACYSMS_IMAGES.'icons/icon-16-edit.png" /></a></div><br />';
		$resultCategory = '<div>'.JText::sprintf('SMS_SELECT_X_EVENTS_FILTERED_X',$nbEventSelection,JText::_('COM_RSEVENTSPRO_SUBMENU_CATEGORIES')).' <a id="listCatLoc_RsEventsPro" class="modal"  onclick="window.acysms_js.openBox(this,\'index.php?option=com_acysms&tmpl=component&ctrl='.$ctrl.'&task=tag&fctplug=chooseCategoryEvents_RsEventsPro&context=tags\');return false;" rel="{handler: \'iframe\', size: {x: 800, y: 500}}"><img class="icon16" src="'.ACYSMS_IMAGES.'icons/icon-16-edit.png" /></a></div><br />';
		...
		$tags['rsevent']->content = "<table class='adminlist table table-striped table-hover' cellpadding='1' width='100%'><tbody>";
		$tags['rsevent']->content .= '<tr><th colspan="4">'.JText::_('SMS_INSERT_INFORMATION').'</th></tr>';
		$tags['rsevent']->content .= "<tr><td>".$typeOfInsert[0].'</td><td>'.$typeOfInsert[1].'</td><td>'.$typeOfInsert[2].'</td><td>'.$typeOfInsert[3]."</td></tr>";
		$tags['rsevent']->content .= '<tr><th colspan="4">'.JText::_('SMS_EVENT_SELECTION').'</th></tr>';
		$tags['rsevent']->content .= '<tr><td colspan="4">'.$eventSelection.'</td></tr>'; // select one event
		$tags['rsevent']->content .= '<tr><td colspan="4">'.$resultCategory.$buttonInsertTag; //select multiple events (the next XX)+button
		$tags['rsevent']->content .= '</td></tr></tbody></table>';



		$tags['rseventauto'] = new stdClass();
		$tags['rseventauto']->name = JText::sprintf('SMS_EVENTS_AUTOMESSAGE_TAGS_X','RsEventsPro');
		...
		$tags['rseventauto']->content = "<table class='adminlist table table-striped table-hover' cellpadding='1' width='100%'><tbody>";
		$k = 0;
		foreach($tableField as $oneField => $title){
			$tags['rseventauto']->content .= '<tr style="cursor:pointer" onclick="insertTag(\'{rsEventsAuto:'.$oneField.'}\')" class="row'.$k.'"><td>'.$oneField.'</td></tr>';
			$k = 1-$k;
		}
		$tags['rseventauto']->content .= '</tbody></table>';
...
<?php }
	...
	function onACYSMSReplaceTags(&$message, $send = true){
		$display = array(
					 'startdate' => JText::_('COM_RSEVENTSPRO_ORDERING_START_DATE').': ',
					 'enddate' =>  JText::_('COM_RSEVENTSPRO_ORDERING_END_DATE').': ',
					 'eventname' => JText::_('COM_RSEVENTSPRO_ORDERING_NAME').': ',
					 'location' => JText::_('COM_RSEVENTSPRO_ORDERING_LOCATION').': '
					 );
	...
	private function _replaceEventTag(&$message,$display)
	{
		...
		foreach ($tags as $tagname => $tag){
			if(empty($tag->type)){
				$message->message_body = str_replace($tagname,'Please select an information to display',$message->message_body);continue;}
			$id = $tag->id;
	...
	private function _replaceEventsAutoTag(&$message, $user)
	{
		...
		foreach ($tags as $tagname => $tag)
		{
			if(empty($tag->id)){$message->message_body = str_replace($tagname,'',$message->message_body);continue;}
			if(empty($user->queue_paramqueue)|| empty($user->queue_paramqueue->eventsList)){$message->message_body = str_replace($tagname,"EVENT-".mb_strtoupper($tag->id)." ",$message->message_body); continue;}
	...
	function onACYSMSDisplayFilterParams_RsEventsPro($message){
		...
		$subscriptionStatus = array(
			'' => ' - - - ',
			'incomplete' => 'COM_RSEVENTSPRO_RULE_STATUS_INCOMPLETE',
			'complete' => 'COM_RSEVENTSPRO_RULE_STATUS_COMPLETE',
			'denied' => 'COM_RSEVENTSPRO_RULE_STATUS_DENIED');
	...
	function onACYSMSSelectData_RsEventsPro(&$acyquery,$message){
		...
		foreach($message->message_receiver['standard']['rseventspro']['subscriptionFilters'] as $oneFilterNumber => $oneFilter){
			...
			if($status == 'complete')	$whereCondition[] = 'rsUser.state = 1';
			else if($status == 'incomplete')	$whereCondition[] = 'rsUser.state = 0';
	...
	function onACYSMSDailyCron(){
		...
		foreach($allMessages as $oneMessage){
			...
			if($time == 'before')	$sendingTime = time() + 86400 + (intval($oneMessage->message_receiver['auto']['rseventspro']['daybefore']) * 86400);
			...
			if($time == 'before')	$senddate = ACYSMS::getTime(date('Y').'-'.$sendingMonth.'-'.$sendingDay.' '.$oneMessage->message_receiver['auto']['rseventspro']['hour'].':'.$oneMessage->message_receiver['auto']['rseventspro']['min']) - (intval($oneMessage->message_receiver['auto']['rseventspro']['daybefore']) * 86400);
