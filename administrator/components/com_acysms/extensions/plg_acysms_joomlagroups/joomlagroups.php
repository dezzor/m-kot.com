...
class plgAcysmsJoomlagroups extends JPlugin
{
	...
	function plgAcysmsJoomlagroups(&$subject, $config){
	...
	function onACYSMSDisplayFiltersSimpleMessage($componentName, &$filters){
		...
		$filters['joomlagroups'] = $newFilter;
	...
	public function onACYSMSdisplayAuthorizedFilters_joomlagroup(&$authorizedFiltersSelection, $conditionNumber){
		...
		$result = '<br />';
		foreach($jGroups as $oneGroup){
			if(!empty($frontEndFilters[$conditionNumber]['filterDetails']['joomlagroup']) && in_array($oneGroup->id,$frontEndFilters[$conditionNumber]['filterDetails']['joomlagroup'])) $checked = 'checked="checked"';
			else $checked = '';
