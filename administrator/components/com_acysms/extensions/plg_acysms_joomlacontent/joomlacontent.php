...
class plgAcysmsJoomlaContent extends JPlugin
{
	...
	function plgAcysmsJoomlaContent(&$subject, $config){
	 ...
	 public function onACYSMSchooseArticle_joomlacontent(&$pageInfo, &$rows, &$categoriesValues){
		...
		if($this->params->get('showOnlyPublished',0))	$filters[] = "article.state = 1";
	...
	private function _replaceContent($tag){
		...
		if(empty($article)){
			...
			if($app->isAdmin()){
				$app->enqueueMessage('The article "'.intval($tag->id).'" could not be loaded','notice');
			}
			return;
		}
	...
	private function _generateAutoMessage(&$message){
		...
		foreach($results as $tagString => $parameter){
			...
			if(!empty($generatingDate)){
				$condition = '`publish_up` >\''.date( 'Y-m-d H:i:s',$generatingDate - date('Z')).'\'';
				$condition .= ' OR `created` >\''.date( 'Y-m-d H:i:s',$generatingDate - date('Z')).'\'';
				$where[] = $condition;
			}
			...
			if(!empty($parameter->max) AND count($allArticles) < $parameter->max){
				...
				$return->message = 'Not enough articles for the tag '.$tagString.' : '.count($allArticles).' / '.$parameter->max.' between '.ACYSMS::getDate($generatingDate).' and '.ACYSMS::getDate($time);
			}
