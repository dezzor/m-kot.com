...
class plgAcysmsSeblod extends JPlugin
{
	...
	function plgAcysmsSeblod(&$subject, $config){
	 ...
	 function onACYSMSGetTags(&$tags) {
	 	...
	 	$tags['seblod'] = new stdClass();
		$tags['seblod']->name = JText::sprintf('SMS_X_USER_INFO','Seblod');
		$db = JFactory::getDBO();
		$tableFields = array_merge(acysms_getColumns('#__cck_store_item_users'), acysms_getColumns('#__cck_store_form_user'));

		$tags['seblod']->content = '<table class="adminlist table table-striped table-hover" cellpadding="1" width="100%"><tbody>';
		$k = 0;
		foreach($tableFields as $oneField => $fieldType){
			$tags['seblod']->content .= '<tr style="cursor:pointer" onclick="insertTag(\'{seblod:'.$oneField.'}\')" class="row'.$k.'"><td>'.$oneField.'</td></tr>';
			$k = 1-$k;
		}
		$tags['seblod']->content .= '</tbody></table>';
	...
	function onACYSMSDisplayFiltersSimpleMessage($componentName, &$filters){
		...
		if($app->isAdmin() || (!$app->isAdmin() && $helperPlugin->allowSendByGroups('seblodfield'))) $filters['seblodField'] = $filter;
