...
class plgSystemAcysmsMijoShopOrders extends JPlugin
{
	...
	function plgSystemAcysmsMijoShopOrders(&$subject, $config){
	...
	function onACYSMSdisplayParamsAutoMessage_mijoshoporders(){
		...
		echo JText::_('SMS_ORDER_CONTAINS_PRODUCT').' : <span id="displayedMijoProduct"/></span><a class="modal"  onclick="window.acysms_js.openBox(this,\'index.php?option=com_acysms&tmpl=component&ctrl=cpanel&task=plgtrigger&plgtype=system&plg=acysmsmijoshoporders&fctName=displayMijoArticles\');return false;" rel="{handler: \'iframe\', size: {x: 800, y: 500}}"><img class="icon16" src="'.ACYSMS_IMAGES.'icons/icon-16-edit.png" /></a>';
	...
	public function onMijoshopBeforeOrderStatusUpdate($data, $order_id, $order_status_id, $notify) {
	 ...
	 function onACYSMSGetTags(&$tags) {
		$tags['mijoshoporders'] = new stdClass();
		$tags['mijoshoporders']->name = JText::sprintf('SMS_X_ORDER_INFO',JText::_('SMS_MIJOSHOP'));
		$db = JFactory::getDBO();
		...
		$tags['mijoshoporders']->content = '<table class="adminlist table table-striped table-hover" cellpadding="1" width="100%"><tbody>';
		$k = 0;
		foreach($tableFields as $oneField => $fieldType){
			$tags['mijoshoporders']->content .= '<tr style="cursor:pointer" onclick="insertTag(\'{mijoshoporders:'.$oneField.'}\')" class="row'.$k.'"><td>'.$oneField.'</td></tr>';
			$k = 1-$k;
		}
		$tags['mijoshoporders']->content .= '</tbody></table>';
	...
	public function checkForAcySMSCredits($order_id, $order_status_id){
		...
		$query = 'SELECT product.product_id AS product_id, orderproduct.quantity AS quantity'.
				' FROM `#__mijoshop_order_product` AS orderproduct INNER JOIN `#__mijoshop_order_product` AS product WHERE orderproduct.order_id = '.intval($order_id);
		$db->setQuery($query);
