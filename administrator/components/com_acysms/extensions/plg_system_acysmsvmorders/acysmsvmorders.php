...
class plgSystemAcysmsvmorders extends JPlugin
{
	...
	function onACYSMSdisplayParamsAutoMessage_virtuemartorders(){
		...
		$to = '<input id="tobuyer" type="radio" name="data[message][message_receiver][auto][virtuemartorders][to]" value="buyer" ><label for="tobuyer">'.JText::_('SMS_BUYER').'</label>
		<input id="tovendor" type="radio" name="data[message][message_receiver][auto][virtuemartorders][to]" value="vendor"><label for="tovendor">'.JText::_('SMS_VENDOR').'</label>
		<input id="toselect" type="radio" name="data[message][message_receiver][auto][virtuemartorders][to]" value="other"><label for="toselect">'.JText::sprintf('SMS_SELECT_USER','</label><span id="test_phone"></span>').'
		<a class="modal" id="selectUser" onclick="window.acysms_js.openBox(this,\'index.php?option=com_acysms&tmpl=component&ctrl=receiver&task=choose&htmlID=selectedVmUser&currentIntegration=virtuemart_2\'); return false;" rel="{handler: \'iframe\', size: {x: 800, y: 500}}"><img class="icon16" src="'.ACYSMS_IMAGES.'icons/icon-16-edit.png" /></a>';
		echo JTEXT::sprintf('SMS_SENDTO_ADDRESS',$to).'<br />';
		echo '<input type ="hidden" name="data[message][message_receiver][auto][virtuemartorders][other][phoneNumber]" id="selectedVmUser"></input>';
		...
		echo JText::_('SMS_ORDER_CONTAINS_PRODUCT').' : <span id="displayedVmProduct"/></span><a class="modal"  onclick="window.acysms_js.openBox(this,\'index.php?option=com_acysms&tmpl=component&ctrl=cpanel&task=plgtrigger&plgtype=system&plg=acysmsvmorders&fctName=displayVmArticles\');return false;" rel="{handler: \'iframe\', size: {x: 800, y: 500}}"><img class="icon16" src="'.ACYSMS_IMAGES.'icons/icon-16-edit.png" /></a>';
	...
	<?php
	 ...
	 function onACYSMSGetTags(&$tags) {
		...
		$tags['virtuemartorders'] = new stdClass();
		$tags['virtuemartorders']->name = JText::sprintf('SMS_X_ORDER_INFO',JText::_('SMS_VIRTUEMART'));
		$db = JFactory::getDBO();
		...
		$tags['virtuemartorders']->content = '<table class="adminlist table table-striped table-hover" cellpadding="1" width="100%"><tbody>';
		$k = 0;
		foreach($tableFields as $oneField => $fieldType){
			$tags['virtuemartorders']->content .= '<tr style="cursor:pointer" onclick="insertTag(\'{virtuemartorders:'.$oneField.'}\')" class="row'.$k.'"><td>'.$oneField.'</td></tr>';
			$k = 1-$k;
		}
		$tags['virtuemartorders']->content .= '</tbody></table>';
	...
	public function manageMessage($data, $old_order_status){
		...
		foreach($allMessages as $messageID => $oneMessage){
			...
			if(isset($oneMessage->message_receiver['auto']['virtuemartorders']['other']['phoneNumber'])&& !empty($oneMessage->message_receiver['auto']['virtuemartorders']['other']['phoneNumber']) && $oneMessage->message_receiver['auto']['virtuemartorders']['to'] =='other'){
			...
			$finalQuery = 'INSERT IGNORE INTO `#__acysms_queue`
							...
							VALUES ("'.intval($oneMessage->message_id).'","'.intval($receiver_id).'", "virtuemart_2", '.intval($senddate).', "0", "3", '.$db->Quote($paramqueue).' )';
	...
	public function checkForAcySMSCredits($order){
		...
		$query = 'SELECT orderitem.virtuemart_product_id AS product_id, orderitem.product_quantity AS quantity'.
				' FROM `#__virtuemart_order_items` AS orderitem WHERE orderitem.virtuemart_order_id = '.intval($order->virtuemart_order_id);
		$db->setQuery($query);
		$allProducts = $db->loadObjectList();

		foreach($allProducts as $oneProduct){
			if($oneProduct->product_id == $AcySMSProductID){
				$joomlaUserId = $order->virtuemart_user_id;
