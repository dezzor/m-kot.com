...
class plgAcysmsFrequencySms extends JPlugin
{
	...
	function plgAcysmsBirthdaysms(&$subject, $config){
	...
	function onACYSMSDisplayParamsAutoMessage_frequencysms(){
		$result = '';
	 	$db = JFactory::getDBO();
		...
		$nbInterval = '<input type="text" value="" name="data[message][message_receiver][auto][frequencysms][nbInterval]"/>';
	 ...
	 function onACYSMSDailyCron(){
		...
		foreach($allMessages as $oneMessage){
			...
			if(empty($oneMessage->message_receiver['auto']['frequencysms']['nbInterval'])){
				if($this->debug)	$this->messages[] = 'No value number defined before the interval (X days for example) for the message : '.$oneMessage->message_id;
				continue;
			}
			...
			if(empty($oneMessage->message_receiver['auto']['frequencysms']['nbInterval'])){
				if($this->debug) $this->messages[] = 'No number defined before the interval selection (X days/weeks/months/)';
				continue;
			}
			...
			if($type == 'field'){
				...
				switch ($oneMessage->message_receiver['auto']['frequencysms']['field']['table']) {
					...
					case 'acysms':
							...
							$frequencyConditions = $this->_addFrequencyConditions('acysmsusers', $oneMessage);
					...
					case 'acymailing_listsub':
							...
							$frequencyConditions = $this->_addFrequencyConditions('acy_listsub', $oneMessage);
					...
					case 'communitybuilder':
							...
							$frequencyConditions = $this->_addFrequencyConditions('comprofiler', $oneMessage);
					...
					case 'hikashop':
							...
							$frequencyConditions = $this->_addFrequencyConditions('hikaaddress', $oneMessage);
					...
					case 'jomsocial':
							...
							$frequencyConditions = $this->_addFrequencyConditions('value', $oneMessage);
					...
					case 'virtuemart_2':
							...
							if(in_array($fieldObject->name,$fields)){
								...
								$frequencyConditions = $this->_addFrequencyConditions('vm', $oneMessage);
							}
						break;
					case 'virtuemart_1':
				...
				if(empty($users)){
					 $this->messages[] = 'Frequency plugin: 0 SMS inserted in the queue for the SMS '.$oneMessage->message_id;
					 continue;
				}
				...
				$querySelect = $acyquery->getQuery(array('DISTINCT '.$oneMessage->message_id.','.$integration->tableAlias.'.'.$integration->primaryField.' , "'.$integration->componentName.'", '.(time()+86400).', '.$config->get('priority_message',3)));
				...
				$this->messages[] = 'Frequency plugin: '.$nbInserted.' birthday SMS inserted in the queue for '.$sendDay.'-'.$sendMonth.'-'.$sendYear.' for the SMS '.$oneMessage->message_subject;
			}
				...
				if($generate){
					...
					$newMessage->message_status = 'notsent';
				...
				$integration->initQuery($acyquery);
				$acyquery->addMessageFilters($oneMessage);
				$querySelect = $acyquery->getQuery(array('DISTINCT '.$newMessage->message_id.','.$integration->tableAlias.'.'.$integration->primaryField.' , "'.$integration->componentName.'", '.(time()+86400).', '.$config->get('priority_message',3)));
				$finalQuery = 'INSERT IGNORE INTO '.ACYSMS::table('queue').' (queue_message_id,queue_receiver_id,queue_receiver_table,queue_senddate,queue_priority) '.$querySelect;
				...
				$db->query();
				$nbInserted = $db->getAffectedRows();
				$this->messages[] = 'Frequency plugin: '.$nbInserted.' frequency SMS inserted in the queue for '.$sendDay.'-'.$sendMonth.'-'.$sendYear.' for the SMS '.$oneMessage->message_subject;
			}
		}
	}
	...
	private function _addFrequencyConditions($prefix, $message, $fieldObject){
		...
		if($interval == 'day'){
			...
			$dayCondition = '';
			if($sendDay == cal_days_in_month(CAL_GREGORIAN,$sendMonth,$sendYear))
				$dayCondition .= ' OR ( DAY("'.$sendDate.'") = (DAY('.$prefix.'.`'.$fieldObject->name.'`)+ INTERVAL 1 DAY)) OR ( DAY("'.$sendDate.'") = (DAY('.$prefix.'.`'.$fieldObject->name.'`)+ INTERVAL 2 DAY)) OR ( DAY("'.$sendDate.'") = (DAY('.$prefix.'.`'.$fieldObject->name.'`)+ INTERVAL 3 DAY)))';
