...
{
	function plgAcysmsMijoshop(&$subject, $config){
	...
	function onACYSMSGetTags(&$tags) {
		...
		$tags['mijoShopUser'] = new stdClass();
		$tags['mijoShopUser']->name = JText::sprintf('SMS_X_USER_INFO','MijoShop');

		$tableFieldsUser = acysms_getColumns('#__mijoshop_user');

		$tags['mijoShopUser']->content = '<table class="adminlist table table-striped table-hover" cellpadding="1" width="100%"><tbody>';
		$k = 0;
		foreach($tableFieldsUser as $oneField => $fieldType){
			$tags['mijoShopUser']->content .= '<tr style="cursor:pointer" onclick="insertTag(\'{mijoShopUser:'.$oneField.'}\')" class="row'.$k.'"><td>'.$oneField.'</td></tr>';
			$k = 1-$k;
		}
		$tags['mijoShopUser']->content .= '</tbody></table>';



	 	$tags['mijoCoupon'] = new stdClass();
		$tags['mijoCoupon']->name = JText::sprintf('SMS_X_COUPON','Mijoshop');
		$prefix = 'mijo';
		...
		$value[1] = JHTML::_('select.option','price',JText::_('COM_MIJOSHOP_SALE_COUPON_TEXT_AMOUNT'));
		...
		$tags['mijoCoupon']->content = '<table class="adminlist table table-striped table-hover" cellpadding="1" width="100%"><tbody>';
		$tags['mijoCoupon']->content .= '<tr> <th>'.JText::_('COM_MIJOSHOP_SALE_COUPON_ENTRY_NAME').'</th> <td colspan="2"><input  maxlength="10" id="'.$prefix.'coupon" type="textbox" value="[key]"/> </td>';
		$tags['mijoCoupon']->content .= '<tr> <th>'.JText::_('COM_MIJOSHOP_SALE_COUPON_COLUMN_DISCOUNT').'</th> <td><input id="'.$prefix.'couponvalue" size="5" type="textbox"/> ';
		$tags['mijoCoupon']->content .= $listCouponValue.'</td></tr>';
		$tags['mijoCoupon']->content .= '<tr> <th>'.JText::_('SMS_EXPIRY_DATE').'</th> <td colspan="2">'.$radioListExpiry.'</td> </tr>';
		$tags['mijoCoupon']->content .= '<tr id="'.$prefix.'expiryDate"> <th>'.JText::_('SMS_FIELD_DATE').'</th> <td colspan="2">'.$timeField[0].$timeField[1].$timeField[2].'</td></tr>';
		$tags['mijoCoupon']->content .= '<tr id="'.$prefix.'expiryDelay" style="display:none"> <th>'.JText::_('SMS_DELAY').'</th> <td colspan="2">'.$displayDelay[0].$displayDelay[1].'</td></tr>';
		$tags['mijoCoupon']->content .= '<tr><td colspan="3"> <input type="button" value="'.JText::_('SMS_INSERT_COUPON').'" onclick="createTagmijoCoupon(\''.$prefix.'\')"/> </td></tr>';
		$tags['mijoCoupon']->content .= '</tbody></table>';
		?>
		...
		function createTagmijoCoupon(prefix)
		...
		foreach($tags as $oneTag)
		{
			...
			if(!empty($couponName))
			{
				...
				if(strlen($couponName)>10)
				{
					...
					$app->enqueueMessage(JText::_('COM_MIJOSHOP_SALE_VOUCHER_ERROR_CODE'), 'error');
				}
	...
	private function _replaceUserTags(&$message,&$user,$send = true){
 	...
 	function onACYSMSDisplayFiltersSimpleMessage($componentName, &$filters){
		...
		if($app->isAdmin() || (!$app->isAdmin() && $helperPlugin->allowSendByGroups('mijoshoporder')))	$filters['mijoshoporder'] = $newFilter;
	}
	...
	function onACYSMSDisplayFilterParams_mijoshoporder($message){
		...
		echo JText::_('SMS_ORDER_CONTAINS_PRODUCT').' : <span id="displayedMijoProduct"/>'.$productName.'</span><a class="modal"  onclick="window.acysms_js.openBox(this,\'index.php?option=com_acysms&tmpl=component&ctrl='.$ctrl.'&task=plgtrigger&plg=mijoshop&fctName=displayMijoArticles\');return false; " rel="{handler: \'iframe\', size: {x: 800, y: 500}}"><img class="icon16" src="'.ACYSMS_IMAGES.'icons/icon-16-edit.png" /></a>';
