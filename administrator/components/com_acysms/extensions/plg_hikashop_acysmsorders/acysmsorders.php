...
class plgHikashopAcysmsorders extends JPlugin
{
		...
		$messageClass = ACYSMS::get('class.message');
		 ...
		 foreach($allMessages as $messageID => $oneMessage){
			...
			if($oneMessage->message_receiver_table == 'hikashop'){
				...
				if(!empty($order->$address_type)) $paramqueue->address_id = $order->$address_type;
				else if(empty($order->$address_type) && !empty($order->old->$address_type)) $paramqueue->address_id = $order->old->$address_type;
				else continue;
			}
			if($oneMessage->message_receiver_table != 'hikashop'){
				if($oneMessage->message_receiver['auto']['hikashoporders']['receiver_type'] == 'buyer'){
					if(!empty($order->order_user_id)) $receiver_id = $order->order_user_id;
					else if(empty($order->order_user_id) && !empty($order->old->order_user_id)) $receiver_id = $order->old->order_user_id;
					else continue;
	...
	public function checkForAcySMSCredits($order){
		...
		$query = 'SELECT product.product_id AS product_id, product.order_product_quantity AS quantity'.
				' FROM `'.hikashop_table('order_product').'` AS product WHERE product.order_id = '.(int)$order->order_id;
		$db->setQuery($query);
		...
		foreach($allProducts as $oneProduct){
			if($oneProduct->product_id == $AcySMSProductID){
				...
				$joomlaUserId = reset($hikaShopIntegration->getJoomUserId($order->order_user_id));
