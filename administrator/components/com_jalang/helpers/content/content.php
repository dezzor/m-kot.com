...
class JalangHelperContent extends JModelList
{
	...
	public function __construct($config = array())
	{
		...
		if (JFile::exists(JPATH_ADMINISTRATOR .'/components/com_mijoshop/mijoshop.xml')) {
	...
	public static function getInstance($itemtype, $prefix = '', $config = array()) {
		...
		if (!isset(self::$instance[$itemtype]) || !self::$instance[$itemtype])
		{
			...
			if(!isset($adapters[$itemtype])) {
				JFactory::getApplication()->enqueueMessage(JText::_('ITEM_TYPE_IS_NOT_SUPPORTED'), 'warning');
				return false;
			}
			...
			if(!class_exists($classname)) {
				JFactory::getApplication()->enqueueMessage(JText::sprintf('CLASS_VAR_IS_NOT_DEFINED', $adapter), 'warning');
				return false;
			}
	...
	{
		JFactory::getApplication()->enqueueMessage(JText::sprintf('THE_VAR_IS_NOT_IMPLEMENTED_FOR_THE_CLASS_VAR', $method, get_class()), 'error');
	}
	...
	/**
	 * Function using for append language code to anyfield we want. duplicate from getnewalias.
	 */
	public function AppendLangToField($alias, $fromLangTag, $toLangTag, $row = null, $generateFrom = '', $makeUnique = false) {
