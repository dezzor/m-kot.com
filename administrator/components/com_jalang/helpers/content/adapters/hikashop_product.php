...
if(JFile::exists(JPATH_ADMINISTRATOR . '/components/com_hikashop/classes/product.php')) {
	//Register if hikashop is installed
	JalangHelperContent::registerAdapter(
		__FILE__,
		'hikashop_product',
		5,
		JText::_('HIKASHOP_PRODUCT'),
		JText::_('HIKASHOP_PRODUCT')
	);
...
// 			$this->reference_fields = array('catid'=>'product_parent_id');
		...
		public function getSortFields()
		{
			return array(
				...
				'a.product_access' => JText::_('JGRID_HEADING_ACCESS'),
		...
		public function updateParent($translator, $sourceid, $row) {
			...
			if (!empty($results)) {
				foreach ($results AS $r) {
					...
					$result = JFactory::getDbo()->insertObject('#__hikashop_variant', $obj);
				}
			}
		...
		public function transferRelatedId($translator, $sourceid, $row) {
			...
			if (!empty($results)) {
				foreach ($results AS $r) {
					...
					$result = JFactory::getDbo()->insertObject('#__hikashop_product_related', $obj);
				}
			}
		}
		...
		public function transferData($table, $where, $ref_field, $value, $and='') {
			...
			if (!empty($results)) {
				foreach ($results AS $r) {
					...
					$obj->{$table.'_id'}=NULL;
					$obj->$ref_field = $value;
					$result = JFactory::getDbo()->insertObject('#__hikashop_'.$table.'', $obj);
				}
			}
		}
	}
