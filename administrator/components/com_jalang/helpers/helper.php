...
class JalangHelper extends JObject
{
	...
	{
		return '<ul class="nav nav-tabs" id="jalang_tabs">
						<li class="'.($vName == 'tool' && $layout == 'default' ? 'active' : '').'">
							<a href="index.php?option=com_jalang&view=tool">'.JText::_('TRANSLATE').'</a></li>
						<li class="'.($vName == 'items' ? 'active' : '').'">
							<a href="index.php?option=com_jalang&view=items">'.JText::_('ASSOCIATION_MANAGER').'</a>
						</li>
						<li class="'.($vName == 'tool' && $layout == 'removelang' ? 'active' : '').'">
							<a href="index.php?option=com_jalang&view=tool&layout=removelang">'.JText::_('DELETE_LANGUAGE_CONTENT').'</a>
						</li>
					</ul>';
		if(self::isJoomla3x()) {
	...
	protected function updateVersion109() {
		return true;
	}
		...
		if (JFile::exists(JPATH_ADMINISTRATOR .'/components/com_easyblog/easyblog.xml')) {
			...
			if ((int)$version >= (int)'5.0.30') {
				...
				$query->update('#__easyblog_revisions')
					...
					->where(''.$db->quoteName('content').' REGEXP "\"language\":\"\\*\""');
				$db->setQuery($query);
				$db->execute();
				...
				$sql = 'SELECT GROUP_CONCAT(id) AS id, '.$db->quoteName('revision_id').' FROM #__easyblog_post
						GROUP BY '.$db->quoteName('revision_id').' having count(*) >= 2';
				$db->setQuery($sql);
