...
if(!ACYSMS_J16){
	...
	class JElementAcysmsGroups extends JElement
	{
		function fetchElement($name, $value, &$node, $control_name)
		{
			...
			$text .= '<a class="modal" id="link'.$control_name.$name.'" title="'.JText::_('Select one or several groups').'"  href="'.$link.'" rel="{handler: \'iframe\', size: {x: 650, y: 375}}"><button class="btn" onclick="return false">'.JText::_('Select').'</button></a>';
	...
	class JFormFieldAcysmsGroups extends JFormField
	{
		...
		function getInput() {
			...
			$text .= '<a class="modal" id="link'.$this->name.'choose" title="'.JText::_('Select one or several groups').'"  href="'.$link.'" rel="{handler: \'iframe\', size: {x: 650, y: 375}}"><button class="btn" onclick="return false">'.JText::_('Select').'</button></a>';
			return $text;
		}
	}
}
