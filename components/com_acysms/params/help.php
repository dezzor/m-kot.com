...
if(version_compare(JVERSION,'1.6.0','<')){
	class JElementHelp extends JElement
	{
		function fetchElement($name, $value, &$node, $control_name)
		{
			...
			$text = '<a class="modal" title="'.JText::_('SMS_HELP',true).'"  href="'.$link.'" rel="{handler: \'iframe\', size: {x: 800, y: 500}}"><button class="btn" onclick="return false">'.JText::_('SMS_HELP').'</button></a>';
			return $text;
		}
	}
}else{
