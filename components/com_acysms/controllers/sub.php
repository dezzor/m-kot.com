...
class SubController extends acySMSController{
	...
	function optin(){
		...
		if((int) $config->get('allow_visitor',1) != 1 && empty($my->id)){
			if($ajax){
				...
				$app->enqueueMessage(JText::_('SMS_ONLY_LOGGED'),'error');
		...
		if(empty($user->user_phone_number) || !$validPhone){
			if($ajax) echo '{"message":"'.JText::_('SMS_VALID_PHONE').'","type":"error","code":"0"}';
		...
		if(($config->get('require_confirmation') && empty($alreadyExists)) ||  ($config->get('require_confirmation') && !empty($alreadyExists->user_id) && !empty($phoneId))) $confirmationRequired = true;
		...
		if(!empty($hiddengroupsstring)){
			...
			foreach($hiddengroups as $id => $idOneGroup){
				if(!isset($currentSubscription[$idOneGroup])){
					...
					if($confirmationRequired)	$addgroups[2][] = $idOneGroup;
		...
		if($ajax){
			...
			if($msgtype == 'success') $app->enqueueMessage($msg,'success');
		...
		if($confirmationRequired){
			$phoneHelper = ACYSMS::get('helper.phone');
			$sendResult = $phoneHelper->sendVerificationCode($user->user_phone_number,'activation_optin');
			if(!$sendResult)	$app->enqueueMessage('Error while sending the AcySMS confirmation code. Please contact the administrator of the website for more informations.','notice');
		}
		...
		if($confirmationRequired) $this->setRedirect(ACYSMS::completeLink('activate&moduleId='.$moduleId.'&userId='.$user->user_id,false,false));
	...
	function optout(){
		...
		if(empty($phoneNumber) || !$validPhone){
			if($ajax) echo '{"message":"'.JText::_('SMS_VALID_PHONE').'","type":"error","code":"7"}';
			else echo "<script>alert('".JText::_('SMS_VALID_PHONE',true)."'); window.history.go(-1);</script>";
			exit;
		}
		...
		if(empty($alreadyExists->user_id)){
			...
			if(empty($redirectUrl)) ACYSMS::display(JText::sprintf('SMS_NOT_IN_GROUP','<b><i>'.$validPhone.'</i></b>'),'warning');
			else $app->enqueueMessage(JText::sprintf('SMS_NOT_IN_GROUP','<b><i>'.$validPhone.'</i></b>'),'notice');
		...
		if(!empty($updategroups)){
			...
			if($config->get('unsubscription_message',1)){
				...
				if(empty($redirectUrl)) ACYSMS::display(JText::_('SMS_UNSUBSCRIPTION_OK'),'info');
				else $app->enqueueMessage(JText::_('SMS_UNSUBSCRIPTION_OK'));
			}
			...
			if(empty($redirectUrl)) ACYSMS::display(JText::_('SMS_UNSUBSCRIPTION_NOT_IN_GROUP'),'info');
			else $app->enqueueMessage(JText::_('SMS_UNSUBSCRIPTION_NOT_IN_GROUP'));
		}
