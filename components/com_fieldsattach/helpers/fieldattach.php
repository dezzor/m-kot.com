...
/**
  ...
  /**
     ...
     */
      ...
      static function getArrayValue($articleid, $category  = false)
      {
        $db = JFactory::getDBO(  );
        $query = 'SELECT `a`.`articleid`,`a`.`fieldsid`,`a`.`value`,`b`.`title` FROM #__fieldsattach_values as a
                  LEFT JOIN #__fieldsattach as b ON  b.id = a.fieldsid  WHERE   (b.language="'. JRequest::getVar("language", "*").'" OR b.language="*" ) AND a.articleid= '.$articleid.' AND `b`.`type`="input" ORDER BY b.ordering';
      ...
      static function getGalleryArrayValue($id, $articleid, $category  = false)
      {
        $db = JFactory::getDBO(  );
        $query = 'SELECT `id` FROM #__fieldsattach
                  WHERE  `type`="imagegallery" and `id`="'.$id.'" ORDER BY `ordering`';

        $db->setQuery( $query );
        $results_fields = $db->loadAssocList();

        foreach($results_fields as $field):
            $query_img = 'SELECT `image1` FROM #__fieldsattach_images
                          WHERE   articleid= '.$articleid.' AND `fieldsattachid`="'.$field['id'].'" ORDER BY `ordering`';
