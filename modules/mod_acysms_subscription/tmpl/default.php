			...
			<table class="acysms_groups">
				<?php foreach($visibleGroupsArray as $myGroupId){
					...
					if($params->get('checkmode',0) == '0' AND !empty($identifiedUser->user_phone_number)){
						if(empty($allGroups[$myGroupId]->groupuser_status)){$check = '';}
						else{
				...
				<tr>
				 ...
				 if($params->get('showterms',false)){
					...
					<td <?php if($displayOutside AND !$displayInline) echo 'colspan="2"'; ?> class="acysubbuttons">
						...
						<input class="button subbutton btn btn-primary" type="submit" value="<?php $subtext = $params->get('subscribetextreg'); if(empty($identifiedUser->user_id) OR empty($subtext)){ $subtext = $params->get('subscribetext',JText::_('SMS_SUBSCRIBE')); } echo $subtext;  ?>" name="Submit" onclick="try{ return submitacysmsform('optin','<?php echo $formName;?>'); }catch(err){alert('The form could not be submitted '+err);return false;}"/>
						<?php }if($params->get('showunsubscribe',false) AND (!$params->get('showsubscribe',true) OR empty($identifiedUser->user_id) OR !empty($countUnsub)) ){ ; ?>
