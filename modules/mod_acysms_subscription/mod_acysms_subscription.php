...
if(strpos($visibleGroups,',') OR is_numeric($visibleGroups)){
	...
	foreach($allGroups as $oneGroup){
		if($oneGroup->group_published){$visibleGroupsArray[] = $oneGroup->group_id;}
	}
}
...
if(strpos($hiddenGroups,',') OR is_numeric($hiddenGroups)){
	...
	$visibleGroupsArray = array();
	foreach($allGroups as $oneGroup){
		if(!empty($oneGroup->group_published)){$hiddenGroupsArray[] = $oneGroup->group_id;}
	}
}
...
if(!empty($identifiedUser->user_id)){
	...
	foreach($visibleGroupsArray as $idOneGroup){
		if($allGroups[$idOneGroup]->groupuser_status == 0) $countSub++;
...
if(strtolower($checkedGroups) == 'all'){ $checkedGroupsArray = $visibleGroupsArray;}
elseif(strpos($checkedGroups,',') OR is_numeric($checkedGroups)){ $checkedGroupsArray = explode(',',$checkedGroups);}
else{ $checkedGroupsArray = array();}
...
$displayedFields = $params->get('customfields','user_firstname,user_lastname,user_phone_number');
...
if(!in_array('user_firstname',$fieldsToDisplay) && empty($connectedUser->user_id)) $fieldsToDisplay[] = 'user_firstname';
if(!in_array('user_lastname',$fieldsToDisplay) && empty($connectedUser->user_id)) $fieldsToDisplay[] = 'user_lastname';
